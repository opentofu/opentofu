...
func (c *Config) ociCredentialsPolicy(ctx context.Context, discoEnv ociauthconfig.ConfigDiscoveryEnvironment) (ociauthconfig.CredentialsConfigs, error) {
	...
	var dockerStyleConfigFiles []string
	var defaultDockerCredentialsHelper string
	if len(c.OCIDefaultCredentials) > 0 {
		...
		defs := c.OCIDefaultCredentials[0]
		discoverAmbientCredentials = defs.DiscoverAmbientCredentials
		dockerStyleConfigFiles = defs.DockerStyleConfigFiles
		defaultDockerCredentialsHelper = defs.DefaultDockerCredentialHelper
	}

	if defaultDockerCredentialsHelper != "" {
		cfgs = append(cfgs, ociauthconfig.NewGlobalDockerCredentialHelperCredentialsConfig(
			"oci_default_credentials block",
			defaultDockerCredentialsHelper,
		))
	}
...
func discoverAmbientOCICredentials(ctx context.Context, dockerStyleConfigFiles []string, discoEnv ociauthconfig.ConfigDiscoveryEnvironment) ([]ociauthconfig.CredentialsConfig, error) {
	switch {
	case dockerStyleConfigFiles != nil: // explicit config file locations
		...
		return cfgs, nil
	default: // automatic discovery using default search locations
