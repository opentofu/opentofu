...
func TestPlan_init_required(t *testing.T) {
	...
	got := output.Stderr()
	if !(strings.Contains(got, "opentf init") && strings.Contains(got, "provider registry.opentf.org/hashicorp/test: required by this configuration but no version is selected")) {
