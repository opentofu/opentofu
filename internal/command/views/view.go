...
func (v *View) Diagnostics(diags tfdiags.Diagnostics) {
	...
	// This filtering is implemented only in here, and not in views.View.Diagnostics because there are meant to be
	// shown only during apply and plan phases. These 2 phases are using this implementation to interact with the user
	// while views.View.Diagnostics is used by other commands that are not meant to show the deprecation diagnostics.
