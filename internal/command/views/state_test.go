...
func testStateHuman(t *testing.T, call func(state State), wantStdout, wantStderr string) {
	...
	stateView := NewState(&arguments.View{ViewType: arguments.ViewHuman, NoColor: true}, view)
	call(stateView)
	output := done(t)
...
func testStateJson(t *testing.T, call func(state State), want []map[string]interface{}, withTimestamp bool) {
	view, done := testView(t)
	stateView := NewState(&arguments.View{ViewType: arguments.ViewJSON, NoColor: true}, view)
	call(stateView)
	output := done(t)
	if output.Stderr() != "" {
...
func testStateMulti(t *testing.T, call func(state State), wantStdout string, wantStderr string, want []map[string]interface{}, withTimestamp bool) {
	...
	view, done := testView(t)
	stateView := NewState(&arguments.View{ViewType: arguments.ViewHuman, JSONInto: jsonInto, NoColor: true}, view)
	call(stateView)
	{
