...
func TestUntaintView(t *testing.T) {
	tests := map[string]struct {
		viewCall   func(v Untaint)
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			testUntaintHuman(t, tc.viewCall, tc.wantStdout, tc.wantStderr)
			testUntaintJson(t, tc.viewCall, tc.wantJson)
			testUntaintMulti(t, tc.viewCall, tc.wantStdout, tc.wantStderr, tc.wantJson)
		})
	}
}

func testUntaintHuman(t *testing.T, call func(v Untaint), wantStdout, wantStderr string) {
	view, done := testView(t)
	v := NewUntaint(arguments.ViewOptions{ViewType: arguments.ViewHuman}, view)
	call(v)
	output := done(t)
...
func testUntaintJson(t *testing.T, call func(v Untaint), want []map[string]interface{}) {
	view, done := testView(t)
	v := NewUntaint(arguments.ViewOptions{ViewType: arguments.ViewJSON}, view)
	call(v)
	output := done(t)
	if output.Stderr() != "" {
...
func testUntaintMulti(t *testing.T, call func(v Untaint), wantStdout string, wantStderr string, want []map[string]interface{}) {
	jsonInto, err := os.CreateTemp(t.TempDir(), "json-into-*")
	if err != nil {
		t.Fatalf("failed to create the file to write json content into: %s", err)
	}
	view, done := testView(t)
	v := NewUntaint(arguments.ViewOptions{ViewType: arguments.ViewHuman, JSONInto: jsonInto}, view)
	call(v)
	{
