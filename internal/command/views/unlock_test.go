...
func testUnlockHuman(t *testing.T, call func(v Unlock), wantStdout, wantStderr string) {
	...
	getView := NewUnlock(&arguments.View{ViewType: arguments.ViewHuman, NoColor: true}, view)
	call(getView)
	output := done(t)
...
func testUnlockJson(t *testing.T, call func(v Unlock), want []map[string]interface{}) {
	view, done := testView(t)
	getView := NewUnlock(&arguments.View{ViewType: arguments.ViewJSON, NoColor: true}, view)
	call(getView)
	output := done(t)
	if output.Stderr() != "" {
...
func testUnlockMulti(t *testing.T, call func(v Unlock), wantStdout string, wantStderr string, want []map[string]interface{}) {
	...
	view, done := testView(t)
	getView := NewUnlock(&arguments.View{ViewType: arguments.ViewHuman, JSONInto: jsonInto, NoColor: true}, view)
	call(getView)
	{
