...
func TestPlanHuman_operation(t *testing.T) {
	...
	defer done(t)
	v := NewPlan(&arguments.View{ViewType: arguments.ViewHuman}, NewView(streams).SetRunningInAutomation(true)).Operation()
...
func TestPlanHuman_hooks(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	defer done(t)
	v := NewPlan(&arguments.View{ViewType: arguments.ViewHuman}, NewView(streams).SetRunningInAutomation((true)))
	hooks := v.Hooks()
