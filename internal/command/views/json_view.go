...
func (v *JSONView) Diagnostics(diags tfdiags.Diagnostics, metadata ...interface{}) {
	...
	for _, diag := range diags {
		diagnostic := jsondiagnostic.NewDiagnostic(diag, sources)
...
func (v *JSONView) PlannedChange(c *jsondiagnostic.ResourceInstanceChange) {
	v.log.Info(
		c.String(),
...
func (v *JSONView) ResourceDrift(c *jsondiagnostic.ResourceInstanceChange) {
	v.log.Info(
...
func (v *JSONView) Outputs(outputs jsondiagnostic.Outputs) {
	v.log.Info(
		outputs.String(),
