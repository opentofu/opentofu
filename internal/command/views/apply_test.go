...
func TestApply_new(t *testing.T) {
	...
	defer done(t)
	v := NewApply(&arguments.View{ViewType: arguments.ViewHuman}, false, NewView(streams).SetRunningInAutomation(true))
	hv, ok := v.(*ApplyHuman)
	if !ok {
...
func TestApplyHuman_outputs(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	v := NewApply(&arguments.View{ViewType: arguments.ViewHuman}, false, NewView(streams))
...
func TestApplyHuman_operation(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	defer done(t)
	v := NewApply(&arguments.View{ViewType: arguments.ViewHuman}, false, NewView(streams).SetRunningInAutomation(true)).Operation()
...
func TestApplyHuman_help(t *testing.T) {
	...
	for name, destroy := range testCases {
		t.Run(name, func(t *testing.T) {
			streams, done := terminal.StreamsForTesting(t)
			v := NewApply(&arguments.View{ViewType: arguments.ViewHuman}, destroy, NewView(streams))
			v.HelpPrompt()
			got := done(t).Stderr()
	...
	for name, tc := range testCases {
		for _, viewType := range views {
			t.Run(fmt.Sprintf("%s (%s view)", name, viewType), func(t *testing.T) {
				streams, done := terminal.StreamsForTesting(t)
				v := NewApply(&arguments.View{ViewType: viewType}, tc.destroy, NewView(streams))
				hooks := v.Hooks()
...
func TestApplyJSON_outputs(t *testing.T) {
	streams, done := terminal.StreamsForTesting(t)
	v := NewApply(&arguments.View{ViewType: arguments.ViewJSON}, false, NewView(streams))
