			...
			wantJson: []map[string]any{
				{
					...
					"@module":           "tofu.ui",
					"type":              "workspace_list",
					"workspaces":        []any{"default", "dev", "prod"},
...
func testWorkspaceHuman(t *testing.T, call func(workspace Workspace), wantStdout, wantStderr string) {
	...
	workspaceView := NewWorkspace(&arguments.View{ViewType: arguments.ViewHuman, NoColor: true}, view)
	call(workspaceView)
	output := done(t)
...
func testWorkspaceJson(t *testing.T, call func(workspace Workspace), want []map[string]interface{}) {
	view, done := testView(t)
	workspaceView := NewWorkspace(&arguments.View{ViewType: arguments.ViewJSON, NoColor: true}, view)
	call(workspaceView)
	output := done(t)
	if output.Stderr() != "" {
...
func testWorkspaceMulti(t *testing.T, call func(workspace Workspace), wantStdout string, wantStderr string, want []map[string]interface{}) {
	...
	view, done := testView(t)
	workspaceView := NewWorkspace(&arguments.View{ViewType: arguments.ViewHuman, JSONInto: jsonInto, NoColor: true}, view)
	call(workspaceView)
	{
