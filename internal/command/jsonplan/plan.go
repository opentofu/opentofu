	...
	for _, rc := range sortedResources {
		...
		if changeV.Before != cty.NilVal {
			...
			if schema.Block.ContainsMarks() {
				valMarks = append(valMarks, schema.Block.ValueMarks(changeV.Before, nil, addr.Resource.Resource, false)...)
			}
		...
		if changeV.After != cty.NilVal {
			...
			if schema.Block.ContainsMarks() {
				valMarks = append(valMarks, schema.Block.ValueMarks(changeV.After, nil, addr.Resource.Resource, false)...)
			}
			if err := ensureEphemeralMarksAreValid(addr, valMarks); err != nil {
				return nil, err
			}
