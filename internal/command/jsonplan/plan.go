...
const (
	...
	ResourceInstanceForgotBecauseLifecycleDestroyInState  = "forgot_because_lifecycle_destroy_in_state"
	ResourceInstanceForgotBecauseLifecycleDestroyInConfig = "forgot_because_lifecycle_destroy_in_config"
)
	...
	for _, rc := range sortedResources {
		...
		if changeV.Before != cty.NilVal {
			...
			if schema.Block.ContainsMarks() {
				valMarks = append(valMarks, schema.Block.ValueMarksExt(changeV.Before, nil, addr.Resource.Resource, false)...)
			}
		...
		if changeV.After != cty.NilVal {
			...
			if schema.Block.ContainsMarks() {
				valMarks = append(valMarks, schema.Block.ValueMarksExt(changeV.After, nil, addr.Resource.Resource, false)...)
			}
			if err := ensureEphemeralMarksAreValid(addr, valMarks); err != nil {
				return nil, err
			}
		...
		case plans.ResourceInstanceForgotBecauseLifecycleDestroyInState:
			r.ActionReason = ResourceInstanceForgotBecauseLifecycleDestroyInState
		case plans.ResourceInstanceForgotBecauseLifecycleDestroyInConfig:
			r.ActionReason = ResourceInstanceForgotBecauseLifecycleDestroyInConfig
		default:
