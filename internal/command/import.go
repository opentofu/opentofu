...
func (c *ImportCommand) Run(args []string) int {
	...
	if travDiags.HasErrors() || c.pedanticMode && (tfdiags.Diagnostics{}).Append(travDiags).HasWarnings() {
	...
	if addrDiags.HasErrors() || c.pedanticMode && addrDiags.HasWarnings() {
		c.registerSynthConfigSource("<import-address>", traversalSrc) // so we can include a source snippet
		c.showDiagnostics(diags)
		c.Ui.Info(importCommandInvalidAddressReference)
		return 1
	...
	if configDiags.HasErrors() || c.pedanticMode && configDiags.HasWarnings() {
		c.showDiagnostics(diags)
		return 1
	}
