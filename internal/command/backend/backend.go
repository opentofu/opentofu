...
// TODO andrei this is defined in 2 more places
const DefaultStateFilename = "terraform.tfstate"

type BackendConfigLoader func(context.Context) (*configs.Backend, tfdiags.Diagnostics)
type BackendCLIOptsBuilder func(context.Context, *BackendOpts) (*backend.CLIOpts, tfdiags.Diagnostics)
...
type BackendFlags struct {
	...
	ConfigLoader      BackendConfigLoader
	CLIOptsBuilder    BackendCLIOptsBuilder
	LegacyStateCb     func()
...
func (m *BackendFlags) BackendConfig(ctx context.Context, opts *BackendOpts) (*configs.Backend, int, tfdiags.Diagnostics) {
	...
	if opts.Config == nil {
		...
		conf, moreDiags := m.ConfigLoader(ctx)
...
func (m *BackendFlags) Backend(ctx context.Context, opts *BackendOpts, enc encryption.StateEncryption) (backend.Enhanced, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	cliOpts, cliOptsDiags := m.CLIOptsBuilder(ctx, opts)
	if cliOptsDiags.HasErrors() {
		return nil, diags.Append(cliOptsDiags)
	}
...
func (m *BackendFlags) BackendFromConfig(ctx context.Context, opts *BackendOpts, enc encryption.StateEncryption) (backend.Backend, tfdiags.Diagnostics) {
	...
	statePath := filepath.Join(m.WorkdirFetcher().DataDir(), DefaultStateFilename)
	...
	m.SetBackendStateCb(nil)
	defer func() {
		if s := sMgr.State(); s != nil && !s.Backend.Empty() {
			m.SetBackendStateCb(s.Backend)
		}
	}()
	...
	case c != nil && !s.Backend.Empty():
		...
		if (uint64(cHash) == s.Backend.Hash) && (!opts.Init || opts.ConfigOverride == nil) {
			...
			if opts.Init && savedBackend != nil {
				if err := m.Workspace.SelectWorkspace(ctx, savedBackend); err != nil {
...
func (m *BackendFlags) BackendForLocalPlan(ctx context.Context, settings plans.Backend, enc encryption.StateEncryption) (backend.Enhanced, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	// If the backend supports CLI initialization, do it.
	if cli, ok := b.(backend.CLI); ok {
		cliOpts, err := m.CLIOptsBuilder(ctx, nil) // TODO andrei not sure that passing nil as opts here is safe
		if err != nil {
			diags = diags.Append(err)
			return nil, diags
		}
	...
	cliOpts, cliOptsDiags := m.CLIOptsBuilder(ctx, nil) // TODO andrei not sure that passing nil opts here is safe
	if cliOptsDiags.HasErrors() {
		diags = diags.Append(cliOptsDiags)
		return nil, diags
	}
...
func (m *BackendFlags) backend_c_r_S(
	ctx context.Context,
...
// specifically.
func (m *BackendFlags) savedBackend(ctx context.Context, sMgr *clistate.LocalState, enc encryption.StateEncryption) (backend.Backend, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
...
func (m *BackendFlags) determineInitReason(previousBackendType string, currentBackendType string, cloudMode cloud.ConfigChangeMode) tfdiags.Diagnostics {
	initReason := ""
	switch cloudMode {
	case cloud.ConfigMigrationIn:
...
func (m *BackendFlags) updateSavedBackendHash(cHash int, sMgr *clistate.LocalState) tfdiags.Diagnostics {
	var diags tfdiags.Diagnostics
...
func (m *BackendFlags) assertSupportedCloudInitOptions(mode cloud.ConfigChangeMode) tfdiags.Diagnostics {
	var diags tfdiags.Diagnostics
...
// errors.
func (m *BackendFlags) backendConfigNeedsMigration(ctx context.Context, c *configs.Backend, s *legacy.BackendState) bool {
	if s == nil || s.Empty() {
...
func (m *BackendFlags) backendInitFromConfig(ctx context.Context, c *configs.Backend, enc encryption.StateEncryption) (backend.Backend, cty.Value, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
...
func (m *BackendFlags) setupEnhancedBackendAliases(b backend.Enhanced) error {
