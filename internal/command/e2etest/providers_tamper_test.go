...
func TestProviderTampering(t *testing.T) {
	...
	pluginDir := filepath.Join(".terraform", "providers", "registry.opentf.org", "hashicorp", "null", providerVersion, getproviders.CurrentPlatform.String())
	...
	t.Run("cache dir totally gone", func(t *testing.T) {
		...
		if want := `registry.opentf.org/hashicorp/null: there is no package for registry.opentf.org/hashicorp/null 3.1.0 cached in ` + providerCacheDir; !strings.Contains(stderr, want) {
	...
	t.Run("null plugin package modified before plan", func(t *testing.T) {
		...
		if want := `registry.opentf.org/hashicorp/null: the cached package for registry.opentf.org/hashicorp/null 3.1.0 (in ` + providerCacheDir + `) does not match any of the checksums recorded in the dependency lock file`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
		if want := `tofu init`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
	...
	t.Run("version constraint changed in config before plan", func(t *testing.T) {
		...
		if want := `provider registry.opentf.org/hashicorp/null: locked version selection 3.1.0 doesn't match the updated version constraints "1.0.0"`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
	...
	t.Run("lock file modified before plan", func(t *testing.T) {
		...
		if want := `provider registry.opentf.org/hashicorp/null: required by this configuration but no version is selected`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
		}
		if want := `tofu init`; !strings.Contains(stderr, want) {
			t.Errorf("missing expected error message\nwant substring: %s\ngot:\n%s", want, stderr)
