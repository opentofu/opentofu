...
func TestEncryptionFlow(t *testing.T) {
	...
	with("required.tf", func() {
		...
		apply().Failure().StderrContains("decrypted payload provided without unencrypted method")
		requireUnencryptedState()
	})
	...
	with("required.tf", func() {
		...
		applyPlan(unencryptedPlan).Failure().StderrContains("decrypted payload provided without unencrypted method")
		requireEncryptedState()
