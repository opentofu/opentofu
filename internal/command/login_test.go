...
func TestLogin(t *testing.T) {
	...
	loginTestCase := func(test func(t *testing.T, c *LoginCommand, ui *cli.MockUi)) func(t *testing.T) {
		return func(t *testing.T) {
			...
			svcs.ForceHostServices(svchost.Hostname("app.opentofu.org"), map[string]interface{}{
				// This represents OpenTofu Cloud (:, which does not yet support the
	...
	t.Run("app.opentofu.org (no login support)", loginTestCase(func(t *testing.T, c *LoginCommand, ui *cli.MockUi) {
		...
		status := c.Run([]string{"app.opentofu.org"})
		if status != 0 {
			t.Fatalf("unexpected error code %d\nstderr:\n%s", status, ui.ErrorWriter.String())
		}

		credsSrc := c.Services.CredentialsSource()
		creds, err := credsSrc.ForHost(svchost.Hostname("app.opentofu.org"))
		if err != nil {
