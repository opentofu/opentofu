...
func DiagnosticFromJSON(diag *jsonentities.Diagnostic, color *colorstring.Colorize, width int) string {
	...
	if diag.Detail != "" {
		...
		if paraWidth > 0 {
			for line := range strings.SplitSeq(FilterControlChars(diag.Detail), "\n") {
...
func appendSourceSnippets(buf *bytes.Buffer, diag *jsonentities.Diagnostic, color *colorstring.Colorize) {
	...
	if diag.Snippet == nil {
		...
		fmt.Fprintf(buf, "  on %s line %d:\n  (source code not available)\n", FilterControlChars(diag.Range.Filename), diag.Range.Start.Line)
	} else {
		...
		fmt.Fprintf(buf, "  on %s line %d%s:\n", FilterControlChars(diag.Range.Filename), diag.Range.Start.Line, contextStr)
		...
		code = fmt.Sprintf(color.Color("%s[underline]%s[reset]%s"), FilterControlChars(before), FilterControlChars(highlight), FilterControlChars(after))
