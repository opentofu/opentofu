...
func Diagnostic(diag tfdiags.Diagnostic, sources map[string]*hcl.File, color *colorstring.Colorize, width int) string {
	return DiagnosticFromJSON(jsondiagnostic.NewDiagnostic(diag, sources), color, width)
}

func DiagnosticFromJSON(diag *jsondiagnostic.Diagnostic, color *colorstring.Colorize, width int) string {
	if diag == nil {
	...
	case jsondiagnostic.DiagnosticSeverityError:
		...
		leftRuleWidth = 2
	case jsondiagnostic.DiagnosticSeverityWarning:
...
func DiagnosticPlain(diag tfdiags.Diagnostic, sources map[string]*hcl.File, width int) string {
	return DiagnosticPlainFromJSON(jsondiagnostic.NewDiagnostic(diag, sources), width)
}

func DiagnosticPlainFromJSON(diag *jsondiagnostic.Diagnostic, width int) string {
	if diag == nil {
		// No good reason to pass a nil diagnostic in here...
		return ""
	}
...
func appendSourceSnippets(buf *bytes.Buffer, diag *jsondiagnostic.Diagnostic, color *colorstring.Colorize) {
	if diag.Address != "" {
	...
	if diag.Snippet == nil {
		...
		if len(snippet.Values) > 0 || (snippet.FunctionCall != nil && snippet.FunctionCall.Signature != nil) {
			...
			values := make([]jsondiagnostic.DiagnosticExpressionValue, len(snippet.Values))
