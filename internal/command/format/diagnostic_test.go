...
func TestDiagnosticFromJSON_invalid(t *testing.T) {
	tests := map[string]struct {
		Diag *jsondiagnostic.Diagnostic
		Want string
	}{
		"zero-value end range and highlight end byte": {
			&jsondiagnostic.Diagnostic{
				Severity: jsondiagnostic.DiagnosticSeverityError,
				Summary:  "Bad end",
				Detail:   "It all went wrong.",
				Range: &jsondiagnostic.DiagnosticRange{
					Filename: "ohno.tf",
					Start:    jsondiagnostic.Pos{Line: 1, Column: 23, Byte: 22},
					End:      jsondiagnostic.Pos{Line: 0, Column: 0, Byte: 0},
				},
				Snippet: &jsondiagnostic.DiagnosticSnippet{
