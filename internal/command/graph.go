...
func (c *GraphCommand) runForExperimentalRuntime(_ context.Context, lr *backend.LocalRun) int {
	...
	if plan := lr.Plan; plan == nil {
		...
		c.Ui.Error("With the experimental new runtime enabled, 'tofu graph' doesn't yet support showing a resource graph. You can save a plan and view its execution graph using -plan=FILENAME, though.")
		return 1
	} else {
