...
func runProviderLockGenericTest(t *testing.T, testDirectory, expected string) {
	...
	// desired result.
	fixtMachineDir := filepath.Join(td, "fs-mirror/registry.opentf.org/hashicorp/test/1.0.0/os_arch")
	wantMachineDir := filepath.Join(td, "fs-mirror/registry.opentf.org/hashicorp/test/1.0.0/", fmt.Sprintf("%s_%s", runtime.GOOS, runtime.GOARCH))
...
func TestProvidersLock_args(t *testing.T) {
	...
	t.Run("invalid provider argument", func(t *testing.T) {
		...
		output := ui.ErrorWriter.String()
		if !strings.Contains(output, "The provider registry.opentf.org/hashicorp/random is not required by the\ncurrent configuration.") {
			t.Fatalf("missing expected error message: %s", output)
		}
	})
}
