...
func prepareStateV4(sV4 *stateV4) (*File, tfdiags.Diagnostics) {
	...
	for _, rsV4 := range sV4.Resources {
		...
		if rsV4.ProviderInstance != "" {
			providerAddr, addrDiags = addrs.ParseAbsProviderInstanceStr(rsV4.ProviderInstance)
			if addrDiags.HasErrors() {
				// If ParseAbsProviderInstanceStr returns an error, the state may have
				...
				providerAddr, legacyAddrDiags = addrs.ParseLegacyAbsProviderInstanceStr(rsV4.ProviderInstance)
		...
		var instanceProviders []addrs.AbsProviderInstance
		...
		for _, isV4 := range rsV4.Instances {
			...
			if isV4.ProviderInstance != "" {
				instanceProvider, instanceProviderKey, addrDiags = addrs.ParseKeyedAbsProviderInstanceStr(isV4.ProviderInstance)
				diags = diags.Append(addrDiags)
	...
	for _, ms := range file.State.Modules {
		...
		for _, rs := range ms.Resources {
			...
			if !hasProviderInstanceKeys {
				providerInstance = rs.ProviderInstance.String()
			}
...
func appendInstanceObjectStateV4(rs *states.Resource, is *states.ResourceInstance, key addrs.InstanceKey, obj *states.ResourceInstanceObjectSrc, deposed states.DeposedKey, isV4s []instanceObjectStateV4, hasProviderInstanceKeys bool) ([]instanceObjectStateV4, tfdiags.Diagnostics) {
	...
	var providerInstance string
	if hasProviderInstanceKeys {
		providerInstance = rs.ProviderInstance.InstanceString(is.ProviderKey)
	}
