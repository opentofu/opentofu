...
func prepareStateV4(sV4 *stateV4) (*File, tfdiags.Diagnostics) {
	...
	for _, rsV4 := range sV4.Resources {
		...
		providerAddr, addrDiags := addrs.ParseAbsProviderInstanceStr(rsV4.ProviderConfig)
		...
		if addrDiags.HasErrors() {
			...
			providerAddr, legacyAddrDiags = addrs.ParseLegacyAbsProviderInstanceStr(rsV4.ProviderConfig)
