...
type instanceCache struct {
	instances map[string]flow.Flow
}
...
func EnableCaching() {
	...
	if cache == nil {
		cache = &instanceCache{
			instances: make(map[string]flow.Flow),
		}
	}
}
...
func (c *instanceCache) cachedOrNewInstance(configKey string, defaultsApply bool) (flow.Flow, error) {
