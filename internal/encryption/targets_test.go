...
func TestBaseEncryption_methodConfigsFromTargetAndSetup(t *testing.T) {
	...
	tests := map[string]btmTestCase{
		...
		"reference-to-undeclared-method-with-fallback": {
			...
			`,
			wantErr: `Test Config Source:12,15-46: Reference to undeclared encryption method; There is no method "aes_gcm" "nonexistent" block declared in the encryption block.`,
			wantMethods: []func(method.Method) bool{
				unencrypted.Is,
			},
		},
	...
	return func(t *testing.T) {
		...
		if diags.HasErrors() {
			if !hasDiagWithMsg(diags, testCase.wantErr) {
				t.Fatalf("Got unexpected error: %v\n\twanted: %v", diags.Error(), testCase.wantErr)
			}
		}
