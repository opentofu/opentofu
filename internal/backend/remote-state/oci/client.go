...
func (c *RemoteClient) Delete() error {
	...
	return c.DeleteAllObjectVersions(c.ctx)
}
func (c *RemoteClient) DeleteAllObjectVersions(ctx context.Context) error {
	...
	response, err := c.objectStorageClient.ListObjectVersions(ctx, request)
	if err != nil {
		return err
	}
	...
	for request.Page != nil {
		...
		listResponse, err := c.objectStorageClient.ListObjectVersions(ctx, request)
		if err != nil {
			return err
		}
	...
	for _, objectVersion := range response.Items {
		...
		_, err := c.objectStorageClient.DeleteObject(ctx, deleteObjectVersionRequest)
		if err != nil {
...
func (c *RemoteClient) Lock(info *statemgr.LockInfo) (string, error) {
	...
	putResponse, putErr := c.objectStorageClient.PutObject(c.ctx, putObjReq)
	if putErr != nil {
		lockInfo, _, err := c.getLockInfo(c.ctx)
		if err != nil {
...
func (c *RemoteClient) Unlock(id string) error {
	...
	lockInfo, etag, err := c.getLockInfo(c.ctx)
	...
	deleteResponse, err := c.objectStorageClient.DeleteObject(c.ctx, deleteRequest)
	if err != nil {
		return &statemgr.LockError{
			Info: lockInfo,
			Err:  err,
