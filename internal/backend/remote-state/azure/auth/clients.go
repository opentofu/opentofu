...
func CloudConfigFromMetadataHost(ctx context.Context, metadataHost string) (cloud.Configuration, error) {
	...
	client := httpclient.New(ctx)
	uri := fmt.Sprintf("https://%s/metadata/endpoints?api-version=2023-11-01", metadataHost)
	...
	if err != nil {
		return cloud.Configuration{}, fmt.Errorf("retrieving environments from Azure MetaData service: %+v", err)
	}
