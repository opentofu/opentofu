...
func (cred *azureCLICredentialAuth) Validate(config *Config) tfdiags.Diagnostics {
	...
	if !config.CLIAuthEnabled {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"You set use_cli to false, which prevents the CLI Auth from being used. Remove this unless you're testing internals or otherwise explicitly want to prevent CLI Authentication.",
		))
		return diags
	}
	...
	// TODO I don't think this works with other environments
