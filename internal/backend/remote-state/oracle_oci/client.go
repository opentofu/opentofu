...
func (c *RemoteClient) Get(contextOb context.Context) (*remote.Payload, error) {
	logger := logWithOperation("download-state-file").Named(c.path)
	logger.Info("Downloading remote state")
	ctx := context.WithValue(contextOb, "logger", logger)
...
func (c *RemoteClient) Put(contextOb context.Context, data []byte) error {
...
func (c *RemoteClient) Delete(ctx context.Context) error {
	...
	return c.DeleteAllObjectVersions(ctx)
}
