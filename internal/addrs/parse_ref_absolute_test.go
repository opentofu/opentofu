...
func TestParseAbsRef(t *testing.T) {
	tests := []struct {
		Input   string
		Want    *AbsReference
		WantErr string
		...
		// Can refer to objects in non-root modules using a special colon
		// delimiter, to disambiguate from referring to a module call in the
		// root module.
		{
			`module.foo:aws_instance.bar`,
			&AbsReference{
				Module: RootModuleInstance.Child("foo", NoKey),
				Subject: Resource{
					Mode: ManagedResourceMode,
					Type: "aws_instance",
					Name: "bar",
		...
		{
			`module.foo[0]:aws_instance.bar`,
			&AbsReference{
				Module: RootModuleInstance.Child("foo", IntKey(0)),
				Subject: Resource{
					Mode: ManagedResourceMode,
					Type: "aws_instance",
					Name: "bar",
		...
		{
			`module.foo["baz"]:aws_instance.bar`,
			&AbsReference{
				Module: RootModuleInstance.Child("foo", StringKey("baz")),
				Subject: Resource{
					Mode: ManagedResourceMode,
					Type: "aws_instance",
					Name: "bar",
		...
		{
			`module.foo["baz"]:module.nested_call`,
			&AbsReference{
				Module: RootModuleInstance.Child("foo", StringKey("baz")),
				Subject: ModuleCall{
					Name: "nested_call",
				...
				SourceRange: tfdiags.SourceRange{
					Start: tfdiags.SourcePos{Line: 1, Column: 1, Byte: 0},
					End:   tfdiags.SourcePos{Line: 1, Column: 37, Byte: 36},
				},
			},
			``,
		},
		{
			// Extra spaces are weird but allowed as usual in HCL.
			` module . foo : aws_instance . bar `,
			&AbsReference{
				Module: RootModuleInstance.Child("foo", NoKey),
				Subject: Resource{
				...
				SourceRange: tfdiags.SourceRange{
					Start: tfdiags.SourcePos{Line: 1, Column: 2, Byte: 1},
					End:   tfdiags.SourcePos{Line: 1, Column: 35, Byte: 34},
				},
			},
			``,
		...
		{
			// The following intentionally includes a problem
			`module.foo[0:]aws_instance.bar`,
			nil,
		...
		{
			// The following intentionally includes a problem
			`module.:aws_instance.bar`,
			nil,
			`Dot must be followed by attribute name.`,
		},
		{
			// The following intentionally includes a problem
			`modlue.foo:aws_instance.bar`,
			nil,
			`A module instance address must begin with "module.".`,
		},
		{
			// The following intentionally includes a problem
			`module.foo[0].extraneous:aws_instance.bar`,
			nil,
	...
	for _, test := range tests {
		t.Run(test.Input, func(t *testing.T) {
			got, diags := ParseAbsRef([]byte(test.Input), "", hcl.InitialPos)
