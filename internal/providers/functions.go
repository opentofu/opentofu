...
func BuildProviderFunction(ctx context.Context, provider Interface, pf addrs.ProviderFunction, stubMissing bool, rng hcl.Range) (function.Function, tfdiags.Diagnostics) {
...
// function call.
func providerFunction(ctx context.Context, name string, spec FunctionSpec, provider Interface) function.Function {
	...
	impl := func(args []cty.Value, retType cty.Type) (cty.Value, error) {
		resp := provider.CallFunction(ctx, CallFunctionRequest{
			Name:      name,
			Arguments: args,
		})

		if argError, ok := resp.Error.(*CallFunctionArgumentError); ok {
...
func providerFunctionParameter(spec FunctionParameterSpec) function.Parameter {
