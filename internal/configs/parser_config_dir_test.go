...
var testRootModuleCall = StaticModuleCall{}
...
func TestParserLoadConfigDirSuccess(t *testing.T) {
	...
	for _, info := range dirs {
		...
		t.Run(name, func(t *testing.T) {
			...
			mod, diags := parser.LoadConfigDir(path, testRootModuleCall)
	...
	for _, info := range files {
		...
		t.Run(fmt.Sprintf("%s as module", name), func(t *testing.T) {
			...
			_, diags := parser.LoadConfigDir("mod", testRootModuleCall)
			if diags.HasErrors() {
...
func TestParserLoadConfigDirWithTests(t *testing.T) {
	...
	for _, directory := range directories {
		t.Run(directory, func(t *testing.T) {
			...
			parser := NewParser(nil)
			mod, diags := parser.LoadConfigDirWithTests(directory, testDirectory, testRootModuleCall)
...
func TestParserLoadConfigDirWithTests_ReturnsWarnings(t *testing.T) {
	parser := NewParser(nil)
	mod, diags := parser.LoadConfigDirWithTests("testdata/valid-modules/with-tests", "not_real", testRootModuleCall)
	if len(diags) != 1 {
...
func TestParserLoadConfigDirFailure(t *testing.T) {
	...
	for _, info := range dirs {
		...
		t.Run(name, func(t *testing.T) {
			...
			_, diags := parser.LoadConfigDir(path, testRootModuleCall)
			if !diags.HasErrors() {
