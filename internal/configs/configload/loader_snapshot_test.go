...
func TestLoadConfigWithSnapshot(t *testing.T) {
	...
	_, got, diags := loader.LoadConfigWithSnapshot(fixtureDir, configs.StaticModuleCall{})
...
func TestLoadConfigWithSnapshot_invalidSource(t *testing.T) {
	...
	_, _, diags := loader.LoadConfigWithSnapshot(".", configs.StaticModuleCall{})
	if !diags.HasErrors() {
		t.Error("LoadConfigWithSnapshot succeeded; want errors", configs.StaticModuleCall{})
	}
}
...
func TestSnapshotRoundtrip(t *testing.T) {
	...
	_, snap, diags := loader.LoadConfigWithSnapshot(fixtureDir, configs.StaticModuleCall{})
	assertNoDiagnostics(t, diags)
	if snap == nil {
		t.Fatalf("snapshot is nil; want non-nil")
	}
	...
	config, diags := snapLoader.LoadConfig(fixtureDir, configs.StaticModuleCall{})
	assertNoDiagnostics(t, diags)
	if config == nil {
