...
func decodeRemovedBlock(removedBlock *parser.Removed) (*Removed, hcl.Diagnostics) {
	...
	if removedBlock.From != nil {
		from, traversalDiags := hcl.AbsTraversalForExpr(removedBlock.From.Expr)
	...
	var seenLifecycle *parser.RemovedLifecycle
	for _, block := range removedBlock.Lifecycle {
		...
		if block.Destroy != nil {
			removed.Destroy = *block.Destroy
			removed.DestroySet = true
		}
	}
	for _, block := range removedBlock.Provisioners {
