...
// runAgainstMultipleIdentifiers runs the given test against a hard list of identifiers ("all", "core:all") and against
// the given identifiers.
// Each test generated by this creates a new context where each entry of lintIdentifiers is included in the
// context.Context as linting hint and gives that ctx to the execution function to run the linting rule against.
func runAgainstMultipleIdentifiers(t *testing.T, f func(t *testing.T, ctx context.Context), lintIdentifiers ...[]linting.RuleAddr) {
	...
	for _, ids := range includedIdentifiers {
		var tcName strings.Builder
		for i, identifier := range ids {
			tcName.WriteString(identifier.String())
			if i < len(ids)-1 {
				tcName.WriteString("_")
			}
		}
		t.Run(tcName.String(), func(t *testing.T) {
			newCtx := tfdiags.ContextWithLintFilterHints(t.Context(), collections.NewSet(ids...), nil)
			f(t, newCtx)
		})
	}
}
