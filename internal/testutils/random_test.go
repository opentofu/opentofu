...
func TestDeterministicRandomID(t *testing.T) {
	...
	if id := testutils.DeterministicRandomID(
		t,
		idLength,
		testutils.CharacterSpaceAlphaNumericUpperLower,
	); id != "MFXow4tIaSnd" {
		t.Fatalf(
...
func TestRandomIDPrefix(t *testing.T) {
	...
	const idLength = 12
	id := testutils.RandomIDPrefix(testPrefix, idLength, testutils.CharacterSpaceAlphaNumericUpperLower)
...
func TestRandomID(t *testing.T) {
	const idLength = 12
	id := testutils.RandomID(idLength, testutils.CharacterSpaceAlphaNumericUpperLower)
	if len(id) != idLength {
		t.Fatalf("Incorrect random ID length: %s", id)
	}
}
