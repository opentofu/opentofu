...
func (i *ModuleInstaller) InstallModules(ctx context.Context, rootDir, testsDir string, upgrade, installErrsOnly bool, hooks ModuleInstallHooks, vars configs.RawVariables) (*configs.Config, tfdiags.Diagnostics) {
	...
	rootMod, mDiags := i.loader.Parser().LoadConfigDirWithTests(rootDir, testsDir, configs.StaticModuleCall{Addr: addrs.RootModule, Raw: vars})
	if rootMod == nil {
		...
		func(req *configs.ModuleRequest) (*configs.Module, *version.Version, hcl.Diagnostics) {
			...
			if !recorded {
				...
				if err == nil && info.IsDir() {
					mod, mDiags := i.loader.Parser().LoadConfigDir(record.Dir, configs.StaticModuleCall{Addr: req.Path, Call: req.Variables})
					if mod == nil {
...
func (i *ModuleInstaller) installLocalModule(req *configs.ModuleRequest, key string, manifest modsdir.Manifest, hooks ModuleInstallHooks) (*configs.Module, hcl.Diagnostics) {
	...
	mod, mDiags := i.loader.Parser().LoadConfigDir(newDir, configs.StaticModuleCall{Addr: req.Path, Call: req.Variables})
	if mod == nil {
...
func (i *ModuleInstaller) installRegistryModule(ctx context.Context, req *configs.ModuleRequest, key string, instPath string, addr addrs.ModuleSourceRegistry, manifest modsdir.Manifest, hooks ModuleInstallHooks, fetcher *getmodules.PackageFetcher) (*configs.Module, *version.Version, hcl.Diagnostics) {
	...
	// Finally we are ready to try actually loading the module.
	mod, mDiags := i.loader.Parser().LoadConfigDir(modDir, configs.StaticModuleCall{Addr: req.Path, Call: req.Variables})
	if mod == nil {
		// nil indicates missing or unreadable directory, so we'll
		// discard the returned diags and return a more specific
