...
func TestModuleInstaller(t *testing.T) {
	...
	_, diags := inst.InstallModules(context.Background(), ".", "tests", false, false, hooks, configs.StaticModuleCall{})
...
func TestModuleInstaller_invalidModuleName(t *testing.T) {
	...
	_, diags := inst.InstallModules(context.Background(), dir, "tests", false, false, hooks, configs.StaticModuleCall{})
	if !diags.HasErrors() {
		t.Fatal("expected error")
	} else {
		assertDiagnosticSummary(t, diags, "Invalid module instance name")
...
func TestModuleInstaller_ExactMatchPrerelease(t *testing.T) {
	...
	defer close()
	inst := NewModuleInstaller(modulesDir, loader, registry.NewClient(nil, nil))
	cfg, diags := inst.InstallModules(context.Background(), ".", "tests", false, false, hooks, configs.StaticModuleCall{})
...
func TestModuleInstaller_fromTests(t *testing.T) {
	...
	// Make sure the configuration is loadable now.
	// (This ensures that correct information is recorded in the manifest.)
	config, loadDiags := loader.LoadConfigWithTests(".", "tests", configs.StaticModuleCall{})
	assertNoDiagnostics(t, tfdiags.Diagnostics{}.Append(loadDiags))
