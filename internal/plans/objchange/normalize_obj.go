...
func normalizeObjectFromLegacySDK(val cty.Value, schema *configschema.Block) cty.Value {
	...
	for name, blockS := range schema.BlockTypes {
		...
		if ty := blockS.ImpliedType(); ty.HasDynamicTypes() {
			vals[name] = lv
			continue
		}
...
func normalizeNestedBlockFromLegacySDKList(val cty.Value, blockS *configschema.NestedBlock) cty.Value {
	...
	case val.IsNull() || val.LengthInt() == 0:
		return cty.ListValEmpty(blockS.ImpliedType())
	default:
...
func normalizeNestedBlockFromLegacySDKSet(val cty.Value, blockS *configschema.NestedBlock) cty.Value {
	...
	case val.IsNull() || val.LengthInt() == 0:
		return cty.SetValEmpty(blockS.ImpliedType())
	default:
		subVals := make([]cty.Value, 0, val.LengthInt())
		for it := val.ElementIterator(); it.Next(); {
			_, subVal := it.Element()
...
func unknownBlockStub(schema *configschema.Block) cty.Value {
	...
	for name, blockS := range schema.BlockTypes {
		...
		case configschema.NestingMap:
			...
			vals[name] = cty.MapValEmpty(blockS.ImpliedType())
		}
	}
	return cty.ObjectVal(vals)
}
