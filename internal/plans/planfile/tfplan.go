...
func readTfplan(r io.Reader) (*plans.Plan, error) {
	...
	// For plans generated by the experimental new language runtime only, we
	// propagate an opaque representation of the execution graph so that
	// the apply phase can use it directly without repeating all of the
	// graph-building logic again.
	//
	// This field is always unset for plans generated by the traditional
	// OpenTofu language runtime.
	plan.ExecutionGraph = rawPlan.TempExecutionGraph
	plan.Destroying = rawPlan.Destroying
...
func writeTfplan(plan *plans.Plan, w io.Writer) error {
	...
	// This field is always unset for plans generated by the traditional
	// OpenTofu language runtime.
	rawPlan.TempExecutionGraph = plan.ExecutionGraph
	rawPlan.Destroying = plan.Destroying
