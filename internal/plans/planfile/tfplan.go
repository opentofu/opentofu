...
// elsewhere. The internal/planfile package is the only
// one that should import the ../internal/planproto package, which
// contains the ugly stubs generated by the protobuf compiler.
// ---------------------------------------------------------------------------

// readPlan reads a protobuf-encoded description from the plan portion of
// a plan file, which is stored in a special file in the archive called
// "tfplan".
func readPlan(r io.Reader) (*plans.Plan, error) {
...
// writePlan serializes the given plan into the protobuf-based format used
// for the "tfplan" portion of a plan file.
func writePlan(plan *plans.Plan, w io.Writer) error {
	if plan == nil {
