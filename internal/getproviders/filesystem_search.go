...
func SearchLocalDirectory(baseDir string) (map[addrs.Provider]PackageMetaList, error) {
	...
	err := filepath.Walk(baseDir, func(fullPath string, info os.FileInfo, err error) error {
		...
		// Unpacked: registry.opentofu.org/opentofu/aws/2.0.0/linux_amd64 (a directory)
		// Packed:   registry.opentofu.org/opentofu/aws/terraform-provider-aws_2.0.0_linux_amd64.zip (a file)
		//
