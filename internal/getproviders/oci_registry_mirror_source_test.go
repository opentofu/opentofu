...
func TestOCIRegistryMirrorSource(t *testing.T) {
	...
	for patchVersion := range 3 {
		...
		createOCITag(t, version.String(), indexDesc, store)
	}
	...
	t.Run("container image instead of provider", func(t *testing.T) {
		...
		if got, want := err.Error(), `unsupported OCI artifact type`; got != want {
			t.Errorf("wrong error\ngot:  %s\nwant: %s", got, want)
		}
	})
	...
	t.Run("valid manifests but unsupported archive format", func(t *testing.T) {
		...
		if got, want := err.Error(), `image manifest contains no "application/vnd.opentofu.providerpkg" layers of type "archive/zip", but has other unsupported formats; is this intended for a different version of OpenTofu?`; got != want {
			t.Errorf("wrong error\ngot:  %s\nwant: %s", got, want)
		}
	})
}
