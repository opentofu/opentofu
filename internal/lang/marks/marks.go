...
func (li LintingInfo) ImpureFuncUsage() []LintingDiagInfo {
	var res []LintingDiagInfo
	for _, info := range li {
		if _, ok := info.(*impureFuncUsage); ok {
			res = append(res, info)
		}
	}
	return res
...
// LintingDiagInfo defines the information that a LintingInfo entry must return in order for a linting rule
// to be able to generate valid diagnostics for.
type LintingDiagInfo interface {
	Summary() string
	Detail() string
}

// lintingMark is the actual mark that will be applied to the values generated by functionality of interest
// in the linting system.
type lintingMark struct {
	cause lintingCause
}

// lintingCause is in the same time embedded in the lintingMark and also returned via the LintingInfo as a LintingDiagInfo.
// This offers the option for a mark to contain logic that generates relevant diagnostics summaries and details of the
// root cause.
type lintingCause interface {
	LintingDiagInfo
...
func (i impureFuncUsage) Summary() string {
	return "Impure function used in a location where convergence is typically expected"
}
...
func (i impureFuncUsage) withPath(p cty.Path) lintingCause {
	return &impureFuncUsage{
		fn:   i.fn,
		path: p,
	}
}

// ImpureFuncUsageMark returns a linting mark that should be applied to any value generated by using
// impure functions.
func ImpureFuncUsageMark(fn string) any {
	return lintingMark{cause: &impureFuncUsage{fn: fn}}
}

// ExtractLintingInformationFromValue extracts any linting mark from the given value and returns those to be used
// later to generate linting warnings.
func ExtractLintingInformationFromValue(val cty.Value) (cty.Value, LintingInfo) {
	var li LintingInfo
	newVal, _ := val.WrangleMarksDeep(func(mark any, path cty.Path) (ctymarks.WrangleAction, error) {
		lm, ok := mark.(lintingMark)
		if !ok {
...
func DropLintingMarks(val cty.Value) cty.Value {
	ret, _ := val.WrangleMarksDeep(func(mark any, path cty.Path) (ctymarks.WrangleAction, error) {
		_, ok := mark.(lintingMark)
		if !ok {
			return nil, nil // no changes to non-deprecation marks
		}
		return ctymarks.WrangleDrop, nil // discard any deprecation marks
