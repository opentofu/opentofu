...
func TestEvalVariableValidations_deprecationDiagnostics(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"local-mod-called-from-root": {
			...
			warnLevel: ParseDeprecatedWarningLevel("all"),
		},
	...
	t.Run("remote-mod-called-from-root", func(t *testing.T) {
		...
		gotDiags := evalVariableDeprecation(varAddr, varCfg, cfg.Module.ModuleCalls["foo-call"].Source, ctx, ParseDeprecatedWarningLevel("local"), nil)
		if len(gotDiags) != 0 {
