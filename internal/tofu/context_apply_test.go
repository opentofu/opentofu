...
func TestContext2Apply_destroyData(t *testing.T) {
	...
	root.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`provider["registry.opentofu.org/opentofu/null"]`),
	)
...
module.child:
  aws_instance.foo:
    ID = foo
    provider = provider["registry.opentofu.org/opentofu/aws"].eu
    type = aws_instance
	`)
}
...
template_file.child:
  ID = foo
  provider = provider["registry.opentofu.org/opentofu/template"]
...
func TestContext2Apply_destroyWithProviders(t *testing.T) {
	...
	removed.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`provider["registry.opentofu.org/opentofu/aws"].baz`),
	)
	...
	// correct the state
	state.Modules["module.mod.module.removed"].Resources["aws_instance.child"].ProviderConfig = mustProviderConfig(`provider["registry.opentofu.org/opentofu/aws"].bar`)
...
func TestContext2Apply_providersFromState(t *testing.T) {
	...
	aliasRoot.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`provider["registry.opentofu.org/opentofu/aws"].bar`),
	)
	...
	moduleProviderRoot.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`module.child.provider["registry.opentofu.org/opentofu/aws"]`),
	)
...
func TestContext2Apply_ProviderMeta_plan_setNoSchema(t *testing.T) {
	...
	for _, diag := range diags {
		if diag.Description().Summary != "Provider registry.opentofu.org/opentofu/test doesn't support provider_meta" {
