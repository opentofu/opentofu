...
func TestContext2Apply_basic(t *testing.T) {
	...
	state, diags := ctx.Apply(context.Background(), plan, m, ParseDeprecatedWarningLevel("all"))
	if diags.HasErrors() {
...
func TestContext2Apply_stop(t *testing.T) {
	...
	go func(plan *plans.Plan) {
		state, _ := ctx.Apply(context.Background(), plan, m, ParseDeprecatedWarningLevel("all"))
		stateCh <- state
	}(plan)
...
func TestContext2Apply_createBeforeDestroy_hook(t *testing.T) {
	...
	if _, diags := ctx.Apply(context.Background(), plan, m, ParseDeprecatedWarningLevel("all")); diags.HasErrors() {
...
func TestContext2Apply_destroyData(t *testing.T) {
	...
	newState, diags := ctx.Apply(context.Background(), plan, m, ParseDeprecatedWarningLevel("all"))
	if diags.HasErrors() {
		t.Fatalf("diags: %s", diags.Err())
	}
...
func TestContext2Apply_destroyModuleVarProviderConfig(t *testing.T) {
	...
	_, diags = ctx.Apply(context.Background(), plan, m, ParseDeprecatedWarningLevel("all"))
	if diags.HasErrors() {
		t.Fatalf("diags: %s", diags.Err())
	}
}
...
func TestContext2Apply_countDecrease(t *testing.T) {
	...
	s, diags := ctx.Apply(context.Background(), plan, m, ParseDeprecatedWarningLevel("all"))
	assertNoErrors(t, diags)
...
func TestContext2Apply_outputOrphanModule(t *testing.T) {
	...
	state, diags = ctx.Apply(context.Background(), plan, emptyConfig, ParseDeprecatedWarningLevel("all"))
	if diags.HasErrors() {
		t.Fatalf("diags: %s", diags.Err())
	}
...
func TestContext2Apply_Provisioner_Diff(t *testing.T) {
	...
	state2, diags := ctx.Apply(context.Background(), plan, m, ParseDeprecatedWarningLevel("all"))
	if diags.HasErrors() {
		logDiagnostics(t, diags)
		t.Fatal("apply failed")
	}
...
func TestContext2Apply_outputAdd(t *testing.T) {
	...
	state1, diags := ctx1.Apply(context.Background(), plan1, m1, ParseDeprecatedWarningLevel("all"))
	if diags.HasErrors() {
		t.Fatalf("diags: %s", diags.Err())
	}
	...
	state2, diags := ctx2.Apply(context.Background(), plan2, m2, ParseDeprecatedWarningLevel("all"))
	if diags.HasErrors() {
		t.Fatalf("diags: %s", diags.Err())
	}
...
func TestContext2Apply_targetedDestroy(t *testing.T) {
	...
	var state *states.State
	{
		...
		// First plan and apply a create operation
		if diags := ctx.Validate(context.Background(), m, ParseDeprecatedWarningLevel("all")); diags.HasErrors() {
...
func TestContext2Apply_vars(t *testing.T) {
	...
	diags := ctx.Validate(context.Background(), m, ParseDeprecatedWarningLevel("all"))
	if len(diags) != 0 {
...
func TestContext2Apply_invalidIndexRef(t *testing.T) {
	...
	diags := c.Validate(context.Background(), m, ParseDeprecatedWarningLevel("all"))
	if diags.HasErrors() {
...
resource "test_resource" "foo" {
	...
	assertNoErrors(t, diags)
	stateWithoutSensitive, diags := newCtx.Apply(context.Background(), plan, newModule, ParseDeprecatedWarningLevel("all"))
	assertNoErrors(t, diags)
