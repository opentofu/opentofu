...
func TestContext2Apply_resourceCountOneList(t *testing.T) {
  ...
  ID = foo
  provider = provider["registry.opentf.org/hashicorp/null"]
...
func TestContext2Apply_destroyData(t *testing.T) {
	...
	root.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`provider["registry.opentf.org/hashicorp/null"]`),
	)
...
module.child:
  aws_instance.foo:
    ID = foo
    provider = provider["registry.opentf.org/hashicorp/aws"].eu
    type = aws_instance
	`)
}
...
template_file.child:
  ID = foo
  provider = provider["registry.opentf.org/hashicorp/template"]
...
func TestContext2Apply_destroyWithProviders(t *testing.T) {
	...
	removed.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`provider["registry.opentf.org/hashicorp/aws"].baz`),
	)
	...
	// correct the state
	state.Modules["module.mod.module.removed"].Resources["aws_instance.child"].ProviderConfig = mustProviderConfig(`provider["registry.opentf.org/hashicorp/aws"].bar`)
...
func TestContext2Apply_providersFromState(t *testing.T) {
	...
	aliasRoot.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`provider["registry.opentf.org/hashicorp/aws"].bar`),
	)
	...
	moduleProviderRoot.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`module.child.provider["registry.opentf.org/hashicorp/aws"]`),
	)
...
func TestContext2Apply_ProviderMeta_plan_setNoSchema(t *testing.T) {
	...
	for _, diag := range diags {
		if diag.Description().Summary != "Provider registry.opentf.org/hashicorp/test doesn't support provider_meta" {
...
`})
	...
	root.SetResourceInstanceCurrent(
		...
		mustProviderConfig(`provider["registry.opentf.org/hashicorp/test"]`),
	)
	root.SetResourceInstanceCurrent(
