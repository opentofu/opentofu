...
func TestNodeApplyableProviderExecute_unknownImport(t *testing.T) {
	...
	detail := `Invalid provider configuration: The configuration for provider["registry.opentofu.org/opentofu/foo"] depends on values that cannot be determined until apply.`
	...
	t.Run("valid", func(t *testing.T) {
		...
		node := NodeApplyableProvider{
			NodeAbstractProvider: &NodeAbstractProvider{
				Addr:   mustProviderConfig(`provider["registry.opentofu.org/opentofu/aws"]`),
				Config: config,
			},
		}
