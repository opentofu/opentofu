...
func TestNewContextRequiredVersion(t *testing.T) {
	...
	for i, tc := range cases {
		t.Run(fmt.Sprintf("%d-%s", i, tc.Name), func(t *testing.T) {
			...
			diags = c.Validate(context.Background(), mod, ParseDeprecatedWarningLevel("all"))
...
`
	...
	validateDiags := ctx.Validate(context.Background(), cfg, ParseDeprecatedWarningLevel("all"))
