...
// TestContext2Apply_writeOnlyReturnedWithValue validates
// that a diagnostic is returned when a write-only attribute is returned
// with a non-nil value, which is forbidden.
func TestContext2Apply_writeOnlyReturnedWithValue(t *testing.T) {
	m := testModuleInline(t, map[string]string{
		"main.tf": `
			resource "test_object" "b" {
	...
	if diags.HasErrors() {
		t.Fatalf("unexpected error during plan: %s", diags.Err().Error())
	}
	_, diags = ctx.Apply(t.Context(), plan, m)
	if !diags.HasErrors() {
