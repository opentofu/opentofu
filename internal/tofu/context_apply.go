...
func (c *Context) Apply(ctx context.Context, plan *plans.Plan, config *configs.Config, moduleDeprecatedWarning DeprecatedWarningLevel) (*states.State, tfdiags.Diagnostics) {
	...
	graph, operation, diags := c.applyGraph(plan, config, providerFunctionTracker, moduleDeprecatedWarning)
	if diags.HasErrors() {
		return nil, diags
	}
...
func (c *Context) applyGraph(plan *plans.Plan, config *configs.Config, providerFunctionTracker ProviderFunctionMapping, moduleDeprecatedWarning DeprecatedWarningLevel) (*Graph, walkOperation, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	graph, moreDiags := (&ApplyGraphBuilder{
		...
		ProviderFunctionTracker:   providerFunctionTracker,
		ModuleDeprecationWarnings: moduleDeprecatedWarning,
...
func (c *Context) ApplyGraphForUI(plan *plans.Plan, config *configs.Config, moduleDeprecatedWarning DeprecatedWarningLevel) (*Graph, tfdiags.Diagnostics) {
	// For now though, this really is just the internal graph, confusing
	// implementation details and all.

	var diags tfdiags.Diagnostics

	graph, _, moreDiags := c.applyGraph(plan, config, make(ProviderFunctionMapping), moduleDeprecatedWarning)
	diags = diags.Append(moreDiags)
	return graph, diags
}
