...
func evalVariableDeprecation(
	...
	expr hcl.Expression,
	ctx EvalContext,
	warnLevel DeprecatedWarningLevel,
	moduleSource addrs.ModuleSource) tfdiags.Diagnostics {
	if !config.DeprecatedSet {
		log.Printf("[TRACE] evalVariableDeprecation: variable %q is having no deprecation configured", addr)
		return nil
	}
	if !warnLevel.VariableDeprecationAllowed(moduleSource) {
	...
	val := ctx.GetVariableValue(addr)
	if val.IsNull() {
		log.Printf("[TRACE] evalVariableDeprecation: variable %q is marked as deprecated null value given", addr)
		return nil
	}
