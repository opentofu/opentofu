...
func testModuleWithSnapshot(t *testing.T, name string) (*configs.Config, *configload.Snapshot) {
	...
	_, instDiags := inst.InstallModules(context.Background(), dir, "tests", true, false, initwd.ModuleInstallHooksImpl{}, configs.StaticModuleCall{})
	...
	config, snap, diags := loader.LoadConfigWithSnapshot(dir, configs.StaticModuleCall{})
	if diags.HasErrors() {
		t.Fatal(diags.Error())
	}
...
func testModuleInline(t *testing.T, sources map[string]string) *configs.Config {
	...
	// in a JSON file so that we can load them below.
	inst := initwd.NewModuleInstaller(loader.ModulesDir(), loader, registry.NewClient(nil, nil))
	_, instDiags := inst.InstallModules(context.Background(), cfgPath, "tests", true, false, initwd.ModuleInstallHooksImpl{}, configs.StaticModuleCall{})
	if instDiags.HasErrors() {
		t.Fatal(instDiags.Err())
	}
	...
	config, diags := loader.LoadConfigWithTests(cfgPath, "tests", configs.StaticModuleCall{})
	if diags.HasErrors() {
		t.Fatal(diags.Error())
	}
