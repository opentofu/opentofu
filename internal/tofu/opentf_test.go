...
func testSetResourceInstanceCurrent(module *states.Module, resource, attrsJson, provider string) {
	module.SetResourceInstanceCurrent(
		testhelpers.MustResourceInstanceAddr(resource).Resource,
		&states.ResourceInstanceObjectSrc{
			Status:    states.ObjectReady,
			AttrsJSON: []byte(attrsJson),
		},
		testhelpers.MustProviderConfig(provider),
		addrs.NoKey,
	)
}
...
func testProviderFuncFixed(rp providers.Interface) providers.Factory {
	if p, ok := rp.(*testhelpers.MockProvider); ok {
...
func testProvisionerFuncFixed(rp *testhelpers.MockProvisioner) provisioners.Factory {
