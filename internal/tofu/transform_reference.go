...
func (t attachDataResourceDependsOnTransformer) Transform(_ context.Context, g *Graph) error {
	...
	for _, v := range vertices {
		...
		if depender.ResourceAddr().Resource.Mode != addrs.DataResourceMode { // TODO andrei we might need a new transformer for ephemeral
			continue
		}
...
func (m ReferenceMap) dataDependsOn(depender graphNodeDependsOn) []*addrs.Reference {
	...
	if n, ok := depender.(GraphNodeConfigResource); ok &&
		n.ResourceAddr().Resource.Mode == addrs.DataResourceMode { // TODO andrei check this part in what branch it should be tackled
