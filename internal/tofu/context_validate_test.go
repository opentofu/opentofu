...
func TestContext2Validate_varRefUnknown(t *testing.T) {
	...
	c.Validate(context.Background(), m, ParseDeprecatedWarningLevel("all"))
...
`,
	...
	diags := testContext2(t, &ContextOpts{}).Validate(context.Background(), m, ParseDeprecatedWarningLevel("all"))
	if !diags.HasErrors() {
		t.Fatal("succeeded; want errors")
	}
