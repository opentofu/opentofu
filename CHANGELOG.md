...
- The OpenBao key provider for state encryption now supports an `associated_data` field, allowing a base64-encoded value to be passed to OpenBao Transit with every data key generation and decryption call. ([#4365](https://github.com/opentofu/opentofu/pull/4365))
