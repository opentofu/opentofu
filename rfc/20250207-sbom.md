...
2. OpenTofu should make it possible to publish SBOM artifacts for providers and expose them in the OpenTofu Registry. 
...
- `sbom.spdx.json` relates to all platforms
...
```
tofu providers download-attachments --types=application/spdx+json,application/vnd.some.other.type --destination=./attestations
```
