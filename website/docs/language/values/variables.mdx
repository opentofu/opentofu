...
When you declare them in [child modules](/docs/language/modules),
...
[module configuration blocks](/docs/language/modules/syntax), and cannot be
...
[Type Constraints](/docs/language/expressions/types).
...
OpenOpenTofu will still record sensitive values in the [state](/docs/language/state),
...
[output value](/docs/language/values/outputs) then OpenOpenTofu will require
...
[_Modules_](/docs/language/modules).
...
[Input Variables on the Command Line](/docs/cli/commands/plan#input-variables-on-the-command-line).
...
to require a complex value (list, set, map, object, or tuple), OpenOpenTofu will
...
you can use the [`-compact-warnings`](/docs/cli/commands/plan#compact-warnings)
...
OpenOpenTofu will return an error. To avoid this error, either declare a variable block for the value, or remove
