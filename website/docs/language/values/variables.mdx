...
* [`ephemeral`][inpage-ephemeral] - Limits the usage of the variable only to ephemeral contexts.
...
Setting a variable as `ephemeral`, OpenTofu will prevent its usage in other contexts than the ephemeral ones:
* other ephemeral variables
* `provider` blocks
* [ephemeral resources](../ephemerality/ephemeral-resources.mdx)
* [write-only attributes](../ephemerality/write-only-attributes.mdx)
* [ephemeral outputs](./outputs.mdx#ephemerality)
* [local values](./locals.mdx#ephemerality)
