...
- A root module can use outputs to print certain values in the CLI output after
  running `tofu apply`.
- When using [remote state](/docs/language/state/remote), root module outputs can be
...
valid [identifier](/docs/language/syntax/configuration#identifiers). In a root module, this name is
displayed to the user; in a child module, it can be used to access the output's
value.

The `value` argument takes an [expression](/docs/language/expressions)
...
OpenOpenTofu will hide values marked as sensitive in the messages from
...
Just as with
[resource dependencies](/docs/language/resources/behavior#resource-dependencies),
