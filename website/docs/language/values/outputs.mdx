...
### `ephemerality` — Control what outputs can be stored to state and plan
...
* [ephemeral variables](./variables.mdx#ephemerality)
* [locals](./locals.mdx#ephemerality)
...
Here are the rules around an `ephemeral` output:
* If an output is wanted to return an ephemeral value, the output needs to be configured with the `ephemeral` attribute.
* An `ephemeral=true` configured output can reference also non-ephemeral values, but the value will be converted into an ephemeral one, restricting its further usage.
