...
description: >-
  A opentf_data managed resource allows you to configure provisioners that
...
resource, you can associate them with a `opentf_data`.

Instances of [`opentf_data`](/opentf/language/resources/opentf-data) are treated
...
`opentf_data` resource. You can also use its `input` argument, `triggers_replace` argument, and any
