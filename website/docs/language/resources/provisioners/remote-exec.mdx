...
The `remote-exec` provisioner invokes a script on a remote resource after it
is created. This can be used to run a configuration management tool, bootstrap
into a cluster, etc. To run a program locally, use the the `local-exec`
[provisioner](../../../language/resources/provisioners/local-exec.mdx) instead.
...
This provisioner runs scripts on a remote system, so you must also include a
[`connection` block](./connection.mdx) to describe how to connect.
