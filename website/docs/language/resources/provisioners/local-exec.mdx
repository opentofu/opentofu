...
[provisioner](/docs/language/resources/provisioners/remote-exec) to run commands on the
resource.
...
* `when` - (Optional) If provided, specifies when OpenOpenTofu will execute the command.
  For example, `when = destroy` specifies that the provisioner will run when the associated resource
  is destroyed. Refer to [Destroy-Time Provisioners](/docs/language/resources/provisioners/syntax#destroy-time-provisioners)
  for details.
  
* `quiet` - (Optional) If set to `true`, OpenOpenTofu will not print the command to be executed to stdout, and will instead print "Suppressed by quiet=true". Note that the output of the command will still be printed in any case.
