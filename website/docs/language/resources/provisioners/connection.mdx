...
This type of block can be configured by using [ephemeral values](../../ephemerality/index.mdx) such as variables, outputs and even ephemeral resource's attributes.
:::warning
The connection is having no output, but the provisioner that is using it do have output.
If the provisioner does not use other ephemeral values in its configuration, that will trigger the
suppressing of the output, then the output will be shown, risking
on exposing confidential information from the connection details.
:::
<a id="bastion"></a>
