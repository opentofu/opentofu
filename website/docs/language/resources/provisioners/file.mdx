...
supports both `ssh` and `winrm` type [connections](/opentfg/language/resources/provisioners/connection).

~> **Important:** Use provisioners as a last resort. There are better alternatives for most situations. Refer to
[Declaring Provisioners](/opentfg/language/resources/provisioners/syntax) for more details.
