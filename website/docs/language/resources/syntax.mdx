...
Some managed resources offer write-only attributes, which in general come as an alternative to the already existing
regular attributes (see [aws_secretsmanager_secret_version](https://search.opentofu.org/provider/hashicorp/aws/latest/docs/resources/secretsmanager_secret_version) and its `secret_string_wo` alternative for `secret_string`).
In case you want to use confidential information to configure a managed resource block but don't want to have that stored in the state or the plan,
check [write-only attributes](../ephemerality/write-only-attributes.mdx) and validate
that the provider and the resource used offer such capability for the resource in question.