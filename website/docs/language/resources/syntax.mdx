...
You can conditionally enable or disable a resource using the `lifecycle.enabled` argument. When set to `false`, the resource is excluded from the configuration and any existing infrastructure will be destroyed:
...
For complete details, see the [`lifecycle.enabled` documentation](../../language/meta-arguments/lifecycle.mdx).
