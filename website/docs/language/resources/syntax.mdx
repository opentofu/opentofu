...
## Deprecation of blocks and attributes

Over time, provider authors may need to change the structure of resources provided. Ideally, they will mark attributes and blocks as deprecated before removing them completely. This notifies consumers of the deprecated
fields, providing time to migrate. OpenTofu utilizes the provider's schema to track usage of any deprecated attributes or blocks and will produce a clear warning.
...
By default, deprecated resources will be consolidated by the resource address. In
order to see them individually, `-consolidation-warnings=false` can be used.
