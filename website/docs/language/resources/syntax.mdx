...
## Conditionally Enabling Resources

You can conditionally enable or disable a resource using the `enabled` argument in a `lifecycle` block. When set to `false`, the resource is excluded from the configuration and any existing infrastructure will be destroyed:
...
For complete details, see the [`enabled` argument in a `lifecycle` block](../../language/meta-arguments/enabled.mdx).
