...
It's possible to add `for_each` providers for modules too, but it's a little bit trickier than using with resources: 
...
Notice that a set is being used as the result of a setsubtract function. This is necessary since if we want to remove an entire region, if we just remove the region from `var.regions`, OpenTofu loses the reference to the provider and then it's not able to remove the resources. So the process for removing a region would be:

1. Add the region to be removed on `disabled_regions`;
1. Run plan/apply on the OpenTofu configuration;
1. Remove the region from `regions`. At that point, you can remove from `disabled_regions` as well.
1. Run plan/apply again;
