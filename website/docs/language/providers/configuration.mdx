...
Given the following configuration, for example:
...
The provider "aws" for_each is equal to the resource provider's for_each on "lambda_cloudfront"
on the code above, both being `local.aws_regions`.
This is known to cause errors, since if a region needs to be removed, you would need a few manual steps using `tofu apply -destroy`
in order to remove the resources first and then remove the region provider instance later. It's an error-prone approach that will be 
best shown below on how to deal with it.
:::

### Deleting a provider instance

As explained above, a resource should always be defined using a different `for_each` expression 
that is a subset of the provider configuration:
...
Now, if a region needs to be removed from these resources, a round of changing the `aws_disabled_regions`, running 
`tofu plan` and `tofu apply`, then removing subsequently this region from both variables would be enough.
