...
You can also use ephemeral values now to configure a provider. These can come
from multiple places, like variables, outputs or even ephemeral resources, but
that depends on the dependency graph and how the dependencies are structured
for it. For an example, you can check the [ephemeral constructs example](../ephemerality/index.mdx#usage-example).
