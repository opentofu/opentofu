...
source address is `registry.opentofu.org/opentofu/http` or, more commonly, just
`hashicorp/http`.
...
public registry, so you may see the shortened version `"opentofu/random"` instead
of `"registry.opentofu.org/opentofu/random"`.

:::note
If you omit the `source` argument when requiring a provider,
OpenTofu uses an implied source address of
`registry.opentofu.org/opentofu/<LOCAL NAME>`.
