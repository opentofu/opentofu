...
To make use of the pg remote state in another configuration, use the [`opentf_remote_state` data source](/opentf/language/state/remote-state-data).
...
- `schema_name` - Name of the automatically-managed Postgres schema, default to `opentf_remote_state`. Can also be set using the `PG_SCHEMA_NAME` environment variable.
