...
:::danger
**Important:** OpenTofu prevents using sensitive values (including variables marked as `sensitive = true`) directly in backend configuration. You must use environment variables to supply credentials. Hardcoding credentials or using sensitive variables in the backend block will result in an error.
:::
...
### Using individual `libpq` environment variables

In this approach, connection details (host, database, user, password) come from `libpq` environment variables, while the backend block only sets non-secret options like schema and table names:
...
### Using `PG_CONN_STR` environment variable

Alternatively, you can provide the entire connection string (without password) as an environment variable:
...
```shellsession
$ export PG_CONN_STR=postgres://db.example.com/tofu_backend
$ export PGUSER=user
$ read -s PGPASSWORD
$ export PGPASSWORD
$ tofu init
...
Then supply credentials via environment variables as shown above.

## Using Input Variables with `nonsensitive()` (Not Recommended)

:::danger
**Warning:** This approach will write your credentials in cleartext under the `.terraform/` directory and may also be included in plan files. Only use this if you understand the security implications and have no other option.
:::
