...
The remote backend is unique among all other OpenTofu backends because it can both store state snapshots and execute CLI-driven run workflow operations for TACOS (TF Automation and Collaboration Software) backends if supported by your TACOS provider. It used to be called an "enhanced" backend.
...
The remote backend supports the following OpenTofu commands (it's up to the TACOS provider to implement support for each command):
