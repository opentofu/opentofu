...
This page describes a special `remote` backend, that has nothing to do with remote state backends.

:::note
**We recommend using the [`cloud` built-in integration](../../../cli/cloud/settings.mdx)** instead of `remote` backend. The `cloud` option includes an improved user experience and more features.
:::
