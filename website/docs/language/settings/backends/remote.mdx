...
:::note
**We recommend using the [`cloud` built-in integration](../../../cli/cloud/settings/index.mdx)** instead of this backend. The `cloud` option includes an improved user experience and more features.
:::
...
`credentials` in the [CLI config file](../../../cli/config/config-file/index.mdx#credentials).
:::
...
- `token` - (Optional) The token used to authenticate with the remote backend.
  ...
  [`tofu login`](../../../cli/commands/login.mdx) or manually configuring
  `credentials` in the
  [CLI config file](../../../cli/config/config-file/index.mdx#credentials).
