...
Running `opentfex init` with the backend file:

```sh
opentfex init -backend-config=config.remote.tfbackend
```

### Data Source Configuration

```hcl
data "opentfex_remote_state" "foo" {
  backend = "remote"
...
!> **Warning:**  We recommend using environment variables to supply credentials and other sensitive data. If you use `-backend-config` or hardcode these values directly in your configuration, OpenTF will include these values in both the `.terraform` subdirectory and in plan files. Refer to [Credentials and Sensitive Data](/opentfex/language/settings/backends/configuration#credentials-and-sensitive-data) for details.
...
- `token` - (Optional) The token used to authenticate with the remote backend.
  ...
  [`opentfex login`](/opentfex/cli/commands/login) or manually configuring
  `credentials` in the
  [CLI config file](/opentfex/cli/config/config-file#credentials).
...
->  **Note:** You must use the `name` key when configuring a `opentfex_remote_state`
...
When executing a remote `plan` or `apply` in a [CLI-driven run](/opentfex/cloud-docs/run/cli),
