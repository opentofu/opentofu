...
- To use multiple remote workspaces, set `workspaces.prefix` to a prefix used in
  ...
  mapping multiple OpenTF CLI [workspaces](/docs/language/state/workspaces)
...
If you need to determine whether a run is local or remote in your OpenTF configuration, we recommend using [run environment variables](/docs/cloud-docs/run/run-environment#environment-variables). The example below uses `TFC_RUN_ID`.
