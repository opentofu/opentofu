...
- `max_versions` - (Optional) Maximum number of historical state versions to retain. `0` (default) disables versioning, `>0` enables versioning with that retention limit. Cleanup always runs asynchronously.
