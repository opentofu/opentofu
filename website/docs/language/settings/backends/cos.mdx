...
- `CreateTag`, `DeleteTag`, and `DescribeTags` on the tag key `tencentcloud-opentf-lock`
...
To make use of the COS remote state in another configuration, use the [`opentf_remote_state` data source](/opentf/language/state/remote-state-data).
