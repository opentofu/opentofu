...
If you have the [TCCLI installed](https://www.tencentcloud.com/document/product/1013/33464), you can use the credentials file obtained with `tccli auth login` to authenticate to your `cos` backend (other methods are described [here](https://www.tencentcloud.com/document/product/1013/64709)). This is ordinarily stored in the home directory under `.tccli`, but you can use the backend variable `shared_credentials_dir` if your TenCent credential files are stored elsewhere. You may also use the `profile` variable to choose which profile file to authenticate with.
