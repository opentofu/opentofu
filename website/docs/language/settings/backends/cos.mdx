...
### Assume Role
If provided with an assume role, OpenOpenTofu will attempt to assume this role using the supplied credentials.
Assume role can be provided by adding an `assume_role` block in the cos backend block.

- `assume_role` - (Optional) The `assume_role` block. If provided, OpenOpenTofu will attempt to assume this role using the supplied credentials.
