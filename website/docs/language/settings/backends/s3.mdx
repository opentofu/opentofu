...
* `s3:PutObjectTagging` on `arn:aws:s3:::mybucket/path/to/my/key.tflock` (required for `use_lockfile`)
* `s3:DeleteObject` on `arn:aws:s3:::mybucket/path/to/my/key`

:::info
All of the `*Object*` action targets will require a `*` on the end if `use_lockfile` is used.
:::
