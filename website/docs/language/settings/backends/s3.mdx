...
[`opentf_remote_state` data
source](/opentf/language/state/remote-state-data).

```hcl
data "opentf_remote_state" "network" {
  backend = "s3"
  config = {
...
The `opent_remote_state` data source will return all of the root module
...
```
data.opent_remote_state.network:
  ...
  backend = s3
  config.% = 3
  config.bucket = opent-state-prod
...
!> **Warning:**  We recommend using environment variables to supply credentials and other sensitive data. If you use `-backend-config` or hardcode these values directly in your configuration, OpenTF will include these values in both the `.terraform` subdirectory and in plan files. Refer to [Credentials and Sensitive Data](/opent/language/settings/backends/configuration#credentials-and-sensitive-data) for details.
...
* `key` - (Required) Path to the state file inside the S3 Bucket. When using a non-default [workspace](/opent/language/state/workspaces), the state path will be `/workspace_key_prefix/workspace_name/key` (see also the `workspace_key_prefix` configuration).
...
[OpenTF's workspaces feature](/opent/language/state/workspaces) to switch
...
provider "aws" {
  ...
  assume_role {
    role_arn = "${var.workspace_iam_roles[opent.workspace]}"
  }
}
```
...
source such as [`opent_remote_state`](/opent/language/state/remote-state-data)
...
`opent init` to initialize the backend and establish an initial workspace
...
```
$ opent workspace new staging
Created and switched to workspace "staging"!

...

$ opent workspace new production
...
```
$ opent workspace select staging
$ opent apply
...
```
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Action": "s3:ListBucket",
      "Resource": "arn:aws:s3:::myorg-opent-states"
    },
    {
      "Effect": "Allow",
      "Action": ["s3:GetObject", "s3:PutObject"],
      "Resource": "arn:aws:s3:::myorg-opent-states/myapp/production/tfstate"
    }
  ]
}
```
        ...
        "Condition" : {
          "ForAllValues:StringEquals" : {
            "dynamodb:LeadingKeys" : [
              "myorg-opent-states/myapp/production/tfstate", // during a state lock the full state file is stored with this key
              "myorg-opent-states/myapp/production/tfstate-md5" // after the lock is released a hash of the statefile's contents are stored with this key
            ]
          }
        }
      }
