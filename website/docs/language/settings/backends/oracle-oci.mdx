...
Note that for the access credentials we recommend using a [partial configuration](./configuration.mdx#partial-configuration).
...
use the [`terraform_remote_state` data source](../../state/remote-state-data.mdx).
...
>
> For more guidance, refer to the [Credentials and Sensitive Data](./configuration.mdx#credentials-and-sensitive-data) documentation.
