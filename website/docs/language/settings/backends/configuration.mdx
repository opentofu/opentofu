...
:::danger Warning
We recommend using environment variables to supply credentials and other sensitive data. If you use `-backend-config` or hardcode these values directly in your configuration, OpenOpenTofu will include these values in both the `.terraform` subdirectory and in plan files. This can leak sensitive credentials.
:::
...
- **Interactively**: OpenOpenTofu will interactively ask you for the required
  values, unless interactive input is disabled. OpenOpenTofu will not prompt for
  optional values.
...
naming pattern. OpenOpenTofu will not prevent you from using other names but following
...
OpenOpenTofu will automatically detect any changes in your configuration
and request a [reinitialization](/docs/cli/init). As part of
the reinitialization process, OpenOpenTofu will ask if you'd like to migrate
...
If you're just reconfiguring the same backend, OpenOpenTofu will still ask if you
...
configuration from the file. OpenOpenTofu will detect this like any other
change and prompt you to [reinitialize](/docs/cli/init).

As part of the reinitialization, OpenOpenTofu will ask if you'd like to migrate
