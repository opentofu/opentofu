...
OpenTF uses persisted state data to keep track of the resources it manages. Most non-trivial OpenTF configurations either intergrate with a TACOS (TF Automation and Collaboration Software) or use a backend to store state remotely. This lets multiple people access the state data and work together on that collection of infrastructure resources.
...
Terraform Cloud automatically manages state in the workspaces associated with your configuration. If your configuration includes a [`cloud` block](/opentf/language/settings/terraform-cloud), it cannot include a `backend` block.

To configure a backend, add a nested `backend` block within the top-level
`opent` block. The following example configures the `remote` backend.
...
When you change a backend's configuration, you must run `opent init` again
...
The local backend configuration is different and entirely separate from the `terraform.tfstate` file that contains [state data](/opent/language/state) about your real-world infrastruture. OpenTF stores the `terraform.tfstate` file in your remote backend.
...
provided as part of [the initialization process](/opent/cli/init).
...
- **File**: A configuration file may be specified via the `init` command line.
  ...
  `opent init`. If the file contains secrets it may be kept in
...
top-level attributes, without the need to wrap it in another `opent`
or `backend` block:

```hcl
address = "demo.consul.io"
path    = "example_app/opent_state"
scheme  = "https"
```
...
$ opent init \
    ...
    -backend-config="path=example_app/opent_state" \
...
and request a [reinitialization](/opent/cli/init). As part of
...
If you're using multiple [workspaces](/opent/language/state/workspaces),
...
change and prompt you to [reinitialize](/opent/cli/init).
