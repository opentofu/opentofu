...
The main module of an OpenTF configuration can integrate with a cloud backend to enable its [CLI-driven run workflow](/docs/cloud-docs/run/cli). You only need to configure these settings when you want to use OpenTF CLI to interact with a cloud backend.
...
Refer to [Using the Cloud Backend](/docs/cli/cloud) in the OpenTF CLI documentation for full configuration details, migration instructions, and command line arguments.
