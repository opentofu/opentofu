...
- `experiments`: Specifies a set of experimental language features the module
  is using.
    ...
    If present, the argument must be set to a literal list of keywords
    representing currently-active experiments. The syntax and keyword to use
    will generally be mentioned in the documentation for any active experiment.
    If you are not intending to use any specific experiment in your module
    then you should not set this argument at all.
...
OpenTofu also accepts `provider_meta` blocks for compatibility with modules that
were written for Terraform, but silently ignores them instead of sending their
data to providers.
