...
description: >-
  OpenTofu uses the dependency lock file .teraform.lock.hcl to track and select
...
- [Providers](/docs/language/providers/requirements), which are plugins for OpenTofu
...
OpenOpenTofu will always select the newest available module version that meets
the specified version constraints. You can use an _exact_ version constraint
to ensure that OpenOpenTofu will always select the same module version.
...
you run [the `tofu init` command](/docs/cli/commands/init). You should
...
If a particular provider has no existing recorded selection, OpenOpenTofu will
...
OpenOpenTofu will always re-select that version for installation, even if a
...
OpenOpenTofu will also verify that each package it installs matches at least one
...
regulations, and then trust that OpenOpenTofu will raise an error if a future
...
- If you install a provider from an origin registry which provides checksums
  that are signed with a cryptographic signature, OpenOpenTofu will treat all
...
- If you install a provider for the first time using an alternative
  installation method, such as a filesystem or network mirror, OpenOpenTofu will
  ...
  [the `tofu providers lock` command](/docs/cli/commands/providers/lock),
...
a change to the configured version constraints, OpenOpenTofu will also record
...
- `h1:`: a mnemonic for "hash scheme 1", which is the current preferred hashing
  ...
  Due to the limited scope of the `zh:` scheme, OpenOpenTofu will
...
OpenOpenTofu will add a new hash to an existing provider only if the hash is
...
existing `provider` block for it), OpenOpenTofu will pre-populate the `hashes`
...
will consist primarily of hashes using that scheme, which OpenOpenTofu will then
...
the case where OpenOpenTofu will learn about a `h1:` equivalent only at a later
...
`tofu init` again, OpenOpenTofu will treat it as if it were
