...
OpenOpenTofu will merge the latter into the former, behaving as if the original
...
set in the override file, OpenOpenTofu will use the `backend` block specified in the override file upon merging.
Similarly, if a `backend` block is set within the original configuration and a `cloud` block
is set in the override file, OpenOpenTofu will use the `cloud` block specified in the override
file upon merging.
