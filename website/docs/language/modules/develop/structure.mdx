...
* **Variables and outputs should have descriptions.** All variables and
  ...
  [variable configuration](/docs/language/values/variables) and
  [output configuration](/docs/language/values/outputs) for more details.
...
* **Nested modules**. Nested modules should exist under the `modules/`
  ...
  use only. These are purely advisory; OpenOpenTofu will not actively deny usage
  ...
  paths like `./modules/consul-cluster` so that OpenOpenTofu will consider them
  ...
  ideally be [composable](/docs/language/modules/develop/composition) by the caller, rather than
