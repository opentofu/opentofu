...
OpenOpenTofu will treat that in a similar way as a remote module and copy it into
...
OpenOpenTofu will recognize unprefixed `github.com` URLs and interpret them
...
OpenOpenTofu will recognize unprefixed `bitbucket.org` URLs and interpret them
...
By default, OpenOpenTofu will clone and use the default branch (referenced by
...
If you use the `ssh://` URL scheme then OpenOpenTofu will assume that the colon
...
When you use an HTTP or HTTPS URL, OpenOpenTofu will make a `GET` request to
...
OpenOpenTofu will append an additional query string argument `tofu-get=1` to
...
OpenOpenTofu will extract the archive to obtain the module source tree.
...
OpenOpenTofu will still extract the entire package to local disk, but will read
