...
[a module registry address](#opentf-registry).
...
across multiple configurations, using a OpenTF-specific protocol that
has full support for module versioning.

[Module Registry](https://registry.example.io/) is an index of modules
...
[Consul module for AWS](https://registry.example.io/modules/hashicorp/consul/aws)
...
```hcl
module "consul" {
  source = "app.example.io/example-corp/k8s-cluster/azurerm"
  version = "1.1.0"
}
```
