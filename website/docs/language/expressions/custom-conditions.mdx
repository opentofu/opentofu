...
## Ephemeral values usage

To ensure that information is not disclosed during condition failures, custom conditions blocks can make use of ephemeral values too but only in the `condition` field.
```hcl
...
Using ephemeral values in the `error_message` will not be errored out when the condition passes, but it will fail rendering the message if an ephemeral value is used:
```shell
