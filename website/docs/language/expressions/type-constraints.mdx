...
user provides the tuple `["a", 15, true]`, OpenOpenTofu will internally transform
...
OpenOpenTofu will automatically convert the second and third strings back to the
...
value against a type constraint containing `any`, OpenOpenTofu will attempt to
...
For example, given the type constraint `list(any)`, OpenOpenTofu will examine
...
An optional attribute with a non-`null` default value is guaranteed to never have the value `null` within the receiving module. OpenOpenTofu will substitute the default value both when a caller omits the attribute altogether and when a caller explicitly sets it to `null`, thereby avoiding the need for additional checks to handle a possible null value.
...
The `production` bucket does not specify the index and error documents, and the `archived` bucket omits the website configuration entirely. OpenOpenTofu will use the default values specified in the `bucket` type constraint.
