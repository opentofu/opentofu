...
If supported by your [backend](/docs/language/settings/backends/configuration), OpenOpenTofu will lock your
...
OpenOpenTofu will not continue. You can disable state locking for most commands
with the `-lock` flag but it is not recommended.

If acquiring the lock is taking longer than expected, OpenOpenTofu will output
