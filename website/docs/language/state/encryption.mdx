...
import ExternalCommand from '!!raw-loader!./examples/encryption/externalcommand.tofu'
...
| Option    | Description                                                                          | Min. | Default |
|-----------|--------------------------------------------------------------------------------------|------|---------|
| `command` | External command to run in an array format, one parameter being an item in an array. | 1    |         |

For example, you can configure the external program as follows:

<CodeBlock language="hcl">{ExternalCommand}</CodeBlock>

#### Handling inputs

When the program starts and a decryption key is desired, OpenTofu will write the metadata stored with the encrypted data to the program's standard input. If no decryption key is needed, OpenTofu will write a null input (`null`) to the standard input. The input data for decryption will have the following format:
...
The keys and values are the same the external program wrote on a previous run. The JSON schema for the above input is the following:

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://github.com/opentofu/opentofu/tree/main/internal/encryption/keyprovider/externalcommand/input.schema.json",
...
The external program must create an output that matches the following format:
...
The JSON schema document for the output is the following:
...
An example external program written in Go could be the following:
...
// Input describes the input data structure. This is nil on input if no existing data needs to be decrypted
type Input *Metadata
...
type Output struct {
	...
	// Meta contains the metadata to store alongside the encrypted data. You can store data here you need
	// to reconstruct the decryption key later.
