...
| encryption_context       | Optional map of key-value string pairs sent to AWS KMS as [Encryption Context](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#encrypt_context) with every `GenerateDataKey` and `Decrypt` call. | -    | -                                  |
| encrypted_metadata_alias | Optional identifier to store metadata in the encrypted state/plan files under. Specify this to allow changing the name of a key provider.                    | -    | derived from the key provider name |
