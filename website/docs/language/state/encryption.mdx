...
| associated_data          | Base64-encoded sent with data key generation and decryption.                                                                                                                | N/A  | -                                  |
| associated_data          | Must be a base64-encoded string in config (use `base64encode()` for plain text) sent to OpenBao Transit with data key generation and decryption.                            | N/A  | -                                  |
| encrypted_metadata_alias | Optional identifier to store metadata in the encrypted state/plan files under. Specify this to allow changing the name of a key provider.                                   | -    | derived from the key provider name |
