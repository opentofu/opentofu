...
This key provider uses the [OpenBao Transit Secret Engine](https://github.com/openbao/openbao/blob/main/website/content/docs/secrets/transit/index.mdx#transit-secrets-engine) to generate keys. You can configure it as follows:

| Option                | Description                                                                                                                                                                                                                   | Min. | Default                |
|-----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------|------------------------|
| token *(required)*    | [Authorization Token](https://github.com/openbao/openbao/blob/main/website/content/docs/concepts/tokens.mdx#tokens) to use when accessing OpenBao API. Available as `BAO_TOKEN` environment variable.                         | N/A  | -                      |
| key_name *(required)* | Name of the transit encryption key to use to encrypt/decrypt the datakey. It should be [pre-configured](https://github.com/openbao/openbao/blob/main/website/content/docs/secrets/transit/index.mdx#setup) in OpenBao server. | N/A  | -                      |
