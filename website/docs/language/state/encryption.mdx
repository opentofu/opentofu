...
When the program starts and a decryption key is desired, OpenTofu will write the metadata stored with the encrypted data to the program's standard input. If no decryption key is needed, OpenTofu will write an empty input (`{}`) to the standard input. The input data for decryption will have the following format:
