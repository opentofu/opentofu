...
import OVHcloudKMS from '!!raw-loader!./examples/encryption/ovhcloud_kms.tf'
import OVHcloudKMSAuthToken from '!!raw-loader!./examples/encryption/ovhcloud_kms_auth_token.yaml'
import OVHcloudKMSAuthMTLS from '!!raw-loader!./examples/encryption/ovhcloud_kms_auth_mtls.yaml'
...
| encryption_context       | Optional map of key-value string pairs sent to AWS KMS as [Encryption Context](https://docs.aws.amazon.com/kms/latest/developerguide/encrypt_context.html) with every `GenerateDataKey` and `Decrypt` call. | -    | -                                  |
| encrypted_metadata_alias | Optional identifier to store metadata in the encrypted state/plan files under. Specify this to allow changing the name of a key provider.                    | -    | derived from the key provider name |
...
| additional_authenticated_data      | Base64-encoded [additional authenticated data (AAD)](https://cloud.google.com/kms/docs/additional-authenticated-data) sent with both encrypt and decrypt calls. | -    | -                                  |
| encrypted_metadata_alias           | Optional identifier to store metadata in the encrypted state/plan files under. Specify this to allow changing the name of a key provider.                       | -    | derived from the key provider name |
...
| associated_data          | Base64-encoded string sent to OpenBao Transit with data key generation and decryption providing authenticity protection.                                                    | N/A  | -                                  |
| encrypted_metadata_alias | Optional identifier to store metadata in the encrypted state/plan files under. Specify this to allow changing the name of a key provider.                                   | -    | derived from the key provider name |
...
### OVHcloud KMS (external)

This key provider uses the [OVHcloud Key Management Service](https://www.ovhcloud.com/en/identity-security-operations/key-management-service/) to generate and wrap data keys.
It builds on OpenTofu's built-in [`external` key provider](#external-experimental): the [opentofu-kms-ovhcloud](https://github.com/ovh/opentofu-kms-ovhcloud) binary is not bundled with OpenTofu, so install it and make it available on your `PATH`.

To install it, see the [documentation](https://github.com/ovh/opentofu-kms-ovhcloud#installation).

Configuration is split into two parts:

- **Key selection**: which KMS key to use. Passed as command-line flags in the `external` key provider's `command` block (or as `KMS_KEY_*` environment variables).
- **Authentication**: how authenticate to your KMS.
Configured in a YAML file (`~/.ovh-kms/okms.yaml`, or the path in the `KMS_CONFIG` environment variable) or via `KMS_RESTAPI_*` environment variables.
Two authentication methods are supported: **mTLS** (the default) and **token**.

#### Key selection options

Passed as command-line flags inside the `command` block. Each flag falls back to an environment variable when omitted; the flag wins when both are set.

| Flag         | Environment variable | Description                                               | Required | Default |
|--------------|----------------------|-----------------------------------------------------------|----------|---------|
| `--key-id`   | `KMS_KEY_ID`         | UUID of the service key used to wrap/unwrap the data key. | Yes      | -       |
| `--key-name` | `KMS_KEY_NAME`       | Name attached to the generated data key.                  | No       | -       |
| `--key-bits` | `KMS_KEY_BITS`       | Data key size in bits. Must be `128`, `192` or `256`.     | No       | `256`   |

#### Authentication options

Read from the YAML file under `profiles.<profile>.restapi`, or from the matching environment variable (the environment overrides the file).

| YAML key      | Environment variable   | Description                               | Required               | Default            |
|---------------|------------------------|-------------------------------------------|------------------------|--------------------|
| `endpoint`    | `KMS_RESTAPI_ENDPOINT` | KMS REST API endpoint URL.                | Yes                    | -                  |
| `ca`          | `KMS_RESTAPI_CA`       | Path to a custom CA certificate.          | No                     | System trust store |
| `auth.type`   | `KMS_RESTAPI_TYPE`     | Authentication method: `mtls` or `token`. | No                     | `mtls`             |
| `auth.cert`   | `KMS_RESTAPI_CERT`     | Path to the mTLS client certificate.      | If `auth.type = mtls`  | -                  |
| `auth.key`    | `KMS_RESTAPI_KEY`      | Path to the mTLS client private key.      | If `auth.type = mtls`  | -                  |
| `auth.okmsId` | `KMS_RESTAPI_OKMSID`   | UUID of the KMS instance.                 | If `auth.type = token` | -                  |
| `auth.token`  | `KMS_RESTAPI_TOKEN`    | REST API token.                           | If `auth.type = token` | -                  |

#### Examples

Add an `encryption` block that points the `external` key provider at the binary and selects the KMS key:

<CodeBlock language="hcl">{OVHcloudKMS}</CodeBlock>

Then configure authentication in `~/.ovh-kms/okms.yaml`:

<Tabs>
    <TabItem value="mtls" label="mTLS authentication">
        <CodeBlock language="yaml">{OVHcloudKMSAuthMTLS}</CodeBlock>
    </TabItem>
    <TabItem value="token" label="Token authentication">
        <CodeBlock language="yaml">{OVHcloudKMSAuthToken}</CodeBlock>
    </TabItem>
</Tabs>
