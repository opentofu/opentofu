...
When using a non-local backend, OpenOpenTofu will not persist the state anywhere
...
In the case of an error persisting the state to the backend, OpenOpenTofu will
...
When manually pushing state, OpenOpenTofu will attempt to protect you from
...
- **Differing lineage**: The "lineage" is a unique ID assigned to a state
  ...
  different state. OpenOpenTofu will not allow this.

- **Higher serial**: Every state has a monotonically increasing "serial"
  number. If the destination state has a higher serial, OpenOpenTofu will
