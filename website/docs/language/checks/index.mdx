...
The following example loads the OpenTF website and validates that it returns the expected status code `200`.
...
A cloud backend can automatically validate whether checks in a workspace’s configuration continue to pass after OpenTF provisions new infrastructure.
...
For example, you can [rewrite the above `check` block example](#checks-syntax) to use a postcondition instead. The below code uses a `postcondition` block to validate that the OpenTF website returns the expected status code of `200`.
...
Both the `check` and `postcondition` block examples validate that the OpenTF website returns a `200` status code during a plan or an apply operation. The difference between the two blocks is how each handles failure.
