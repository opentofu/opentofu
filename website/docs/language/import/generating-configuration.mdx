...
In specific cases, the attributes of the generated blocks can be suffixed with a comment. That comment can contain different hints
on what to expect and how to use it.
For example, if the generated `resource` block contains sensitive and write-only attributes, you will something similar to this:
```hcl
