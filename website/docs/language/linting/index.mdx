...
:::info
At the moment of writing this note, linting functionality is under active development and considered experimental.
Based on user feedback and upcoming feature requirements, future changes to the current implementation may introduce
breaking changes to existing functionality. This is also indicated by a warning issued by `tofu` whenever linting is
enabled.
:::
...
```shell
│ Warning: Variable not used (core:unused-variable)
│
│   on main.tf line 17:
...
```shell
│ Warning: Local not used (core:unused-local)
│
│   on main.tf line 9, in locals:
│    9:   arn = "arn:aws:s3:::demo-bucket"
│
│ Found no usage of the local "arn".
```
