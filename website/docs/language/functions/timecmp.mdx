...
[custom condition checks](/docs/language/expressions/custom-conditions) that
...
* [`timestamp`](/docs/language/functions/timestamp) returns the current timestamp when it is evaluated
  during the apply step.
* [`timeadd`](/docs/language/functions/timeadd) can perform arithmetic on timestamps by adding or removing a specified duration.
