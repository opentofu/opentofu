---
description: >-
  An introduction to the functions which can be use to transform and
...
added to the current module's context under `provider::${provider_alias}::${function_name}`.
...
* OpenTofu's provider protocol is compatible with Hashicorp's provider protocol
