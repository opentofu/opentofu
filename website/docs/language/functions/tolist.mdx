...
This function is equivalent to calling [`convert`](./convert.mdx) with a
type constraint of `list(any)`. Use `convert` if you need to specify the element
type instead of letting it be inferred automatically.
