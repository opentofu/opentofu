...
[the `local_file` data source](https://registry.terraform.io/providers/opentofu/local/latest/docs/data-sources/file)
...
* [`filebase64`](/docs/language/functions/filebase64) also reads the contents of a given file,
  but returns the raw bytes in that file Base64-encoded, rather than
  interpreting the contents as UTF-8 text.
* [`fileexists`](/docs/language/functions/fileexists) determines whether a file exists
  at a given path.
* [`templatefile`](/docs/language/functions/templatefile) renders using a file from disk as a
  template.
