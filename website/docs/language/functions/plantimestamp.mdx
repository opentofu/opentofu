...
```hcl
check "opentf.org_certificate" {
  data "tls_certificate" "opentf.org" {
    url = "https://opentf.org/"
  }

  assert {
    condition = timecmp(plantimestamp(), data.tls_certificate.opentf.org.certificates[0].not_after) < 0
    error_message = "opentf.org certificate has expired"
  }
}
```
