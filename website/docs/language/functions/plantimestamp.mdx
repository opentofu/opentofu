...
```hcl
check "opentffoundation{
  data "tls_certificate" "opentffoundation
    url = "https://www.opentffoundation
  }

  assert {
    condition = timecmp(plantimestamp(), data.tls_certificate.opentffoundation0].not_after) < 0
    error_message = "opentffoundationas expired"
  }
}
```
