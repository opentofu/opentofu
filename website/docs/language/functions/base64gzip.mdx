...
:::note
The implementation of this function changed between the OpenTofu v1.12 and v1.13
series. The result for any given input is equivalent but _not_ byte-for-byte
identical between the two releases, and so resource configuration arguments
defined with this function will propose changes when upgrading across that
version boundary.

If you cannot apply the proposed changes immediately then consider using
[the `ignore_changes` meta-argument](../resources/behavior.mdx#ignore_changes)
to cause OpenTofu to temporarily ignore the different function result, which
you can then remove once you're ready to switch to the new encoding.
:::
