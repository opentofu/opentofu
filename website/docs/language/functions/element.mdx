...
To get the last element from the list use [`length`](/docs/language/functions/length) to find
...
* [`index`](/docs/language/functions/index_function) finds the index for a particular element value.
* [`lookup`](/docs/language/functions/lookup) retrieves a value from a _map_ given its _key_.
