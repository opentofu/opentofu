...
If a region needs to be removed, the provider instance key would need to be set as `null` on `aws_regions`,
and after a round of `tofu plan` and `tofu apply`, it could be removed from
the `aws_regions` map.
