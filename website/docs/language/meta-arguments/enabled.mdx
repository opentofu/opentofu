...
In addition to the usage restriction above, when using `enabled` in context of `resource`
and `data` blocks, it cannot use sensitive or ephemeral values either.
These kind of values can be used only when `enabled` is used in the context of
`ephemeral` blocks.
