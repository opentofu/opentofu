...
* `enabled` (bool) - Controls whether a resource is created and managed by OpenTofu.
  ...
  This offers a cleaner alternative to the `count` workaround pattern (`count = var.enabled ? 1 : 0`),
  which involves accessing resources via array indexing for single instances. If an existing resource or
  module uses that pattern, they can switch to `lifecycle.enabled`, and OpenTofu will create an implicit
  move between them.
  ...
  Conditional expressions must be used to handle this:
  ...
  # Using can() - more explicit about null checking
  output "instance_id" {
    value = can(aws_instance.example.id) ? aws_instance.example.id : "not-created"
  }
  ...
  **Restrictions:**
  ...
  The `enabled` argument cannot be used together with `count` or `for_each`, as these
  meta-arguments serve different purposes. Use `enabled` for single-instance resources,
  `count` for creating multiple numbered instances, or `for_each` for creating multiple
  named instances.

  The `enabled` argument cannot use:
