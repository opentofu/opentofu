...
This guide walks you through migrating from Terraform to OpenTofu. OpenTofu is designed as a drop-in replacement for Terraform, and in most cases, your existing Terraform configurations will work without modification.
...
1. **Your Terraform state files**
   - For local state: Copy your `terraform.tfstate` and `terraform.tfstate.backup` files
...
OpenTofu maintains compatibility with Terraform state files. State files can be shared between Terraform and OpenTofu, making migration reversible.

### Configuration compatibility

OpenTofu aims to maintain compatibility with Terraform's HCL configuration language. Most configurations will work without modification.