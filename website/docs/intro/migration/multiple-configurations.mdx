...
This guide addresses the specific challenges of migrating systems built from multiple Intedependent Terraform configurations that share data using the `terraform_remote_state` data source.
...
The `terraform_remote_state` data source allows one terraform configuration to read outputs from another configuration's state file. This creates a dependency relationship between configurations
...
- Configurations migrated to OpenTofu can still read state from configurations remaining on Terraform
...
**Phase 1:** Migrate leaf nodes (no dependents)
```
F, G → (migrate these first)
```

**Phase 2:** Migrate their dependencies
```
B, E → (migrate after F and G)
```
...
Create a visual representation of your configuration dependencies using `terraform_remote_state` data sources.

**Tools to help:**
- Search your codebase for `terraform_remote_state`
- Document which configurations read from which backends/state files
- Draw the dependency graph (use arrows pointing from dependent → dependency)
...
1. Follow the standard [Migration Guide](/docs/intro/migration/migration-guide)
2. Verify the migration with `tofu plan`
3. Apply with `tofu apply`
4. **Test the migration:** Ensure dependent configurations (if any) can still read this configuration's state
...
Your configurations may be using different Terraform versions (1.6.x, 1.7.x, 1.8.x). This is fine:

- **Migrate each configuration** according to its current Terraform version
- **Follow version-specific guidance** as needed
- **The bottom-up order remains the same** regardless of version differences

Each configuration can be migrated independently using its version-specific migration guide, as long as you maintain the bottom-up dependency order.
