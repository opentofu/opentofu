...
As a first step, please [follow the installation instructions for the OpenTofu CLI tool](../install/index.mdx). Make
sure to install OpenTofu 1.6.2. Please test if you can successfully execute the `tofu` command and that it returns the
correct version:
