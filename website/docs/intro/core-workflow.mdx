...
collaborating on infrastructure, and how OpenTF Cloud enables this
...
[Running OpenTF in Automation](/opentf/tutorials/automation/automate-opentf?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS)
guide.
...
![Screenshot of Pull Request with manually posted OpenTF plan output](/img/docs/manually-pasted-plan-output.png)
...
## The Core Workflow Enhanced by Cloud Backends
...
organizations scale. The cloud backends were designed to support and enhance
the core OpenTF workflow for anyone collaborating on infrastructure, from
small teams to large organizations. Let's look at how a cloud backend makes
for a better experience at each step.

### Write

A cloud backend provides a centralized and secure location for storing
input variables and state while also bringing back a tight feedback loop for
speculative plans for config authors. OpenTF configuration can interact with
a cloud backend through the [CLI integration](/opentf/cli/cloud).

```
terraform {
  cloud {
    organization = "my-org"
    hostname = "app.placeholderplaceholderplaceholder.io"
...
After you configure the integration, a cloud backend API key is all your team members need to edit config and run speculative plans
against the latest version of the state file using all the remotely stored
input variables.

```sh
$ opentf workspace select my-app-dev
Switched to workspace "my-app-dev".

$ opentf plan

Running plan remotely in a cloud backend.

Output will stream here. To view this plan in a browser, visit:

https://app.placeholderplaceholderplaceholder.io/my-org/my-app-dev/.../

Refreshing OpenTF state in-memory prior to plan...
...
Once a pull request is ready for review, a cloud backend makes the process
...
full review of the plan, clicking the link to a cloud backend brings up a
view that allows them to quickly analyze the full plan details.

![Screenshot of Pull Request run in a cloud backend](/img/docs/pr-plan.png)
...
After merge, a cloud backend presents the concrete plan to the team for
review and approval.
...
![Screenshot of back-and-forth in a cloud backend comments](/img/docs/plan-comments.png)

Once the Apply is confirmed, a cloud backend displays the progress live
...
a cloud backend introduces new layers that build on this core workflow to
