...
## Step 1: If using an S3 backend - Remove any use of `use_legacy_workflow` from S3 backend configurations, if any
...
**Note: If you are using the S3 backend** - You would need to run `tofu init -reconfigure` to reinitialize the backend.
