...
* `hostname` is the registry host that the provider is considered to have
  originated from, and the default location OpenOpenTofu will consult for
...
If the given URL is a relative URL then OpenOpenTofu will interpret it as relative
