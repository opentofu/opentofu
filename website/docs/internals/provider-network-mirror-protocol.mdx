...
given in the network mirror base URL, OpenOpenTofu will include those credentials
...
* `hashes` (optional): a JSON array of strings containing one or more hash
  ...
  If the response includes at least one hash, OpenOpenTofu will select the hash
  ...
  property then OpenOpenTofu will install the indicated archive with no
  verification.
...
in turn while providing the same output directory each time. OpenOpenTofu will
