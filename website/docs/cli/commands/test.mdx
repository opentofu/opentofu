...
The `test` command executes the automated integration tests following the next steps:
...
the file `foo.tftest.hcl` and `bar.tftest.hcl` will be selected only.
...
the test `suite` will consist of the files: `main.tftest.hcl`, `submodule/foo.tftest.hcl` and `tests/main.tftest.hcl`.
