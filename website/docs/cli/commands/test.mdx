...
The `run` block defines a single test unit. It specifies the operation which OpenTofu will execute, sets "mocks" to overwrite providers,
and defines the checks to assess the OpenTofu state after the operation was executed.

The following non-mutually exclusive blocks and attributes can be defined within the scope of the `run` block to configure it.

| Name              | Type   | Multiple allowed | Description                                                                                                                                                                                     |
|:------------------|:-------|:-----------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `assert`          | block  | true             | Defines the test checks and assertion message. It is the key configuration to assess the OpenTofu configuration behavior against a user defined reference.                                      |
| `variables`       | block  | false            | Defines "local" `run` variables (see [details](/docs/language/expressions/types/#mapsobjects)).                                                                                                 |
| `module`          | block  | false            | Defines module to use during the execution of a given `run` block.                                                             |
...
:::info
Note that if you would combine several assertions in a single `run` and some of them fail, the total numer of failed tests will be reported as 1.
The opposite is also true, 1 will be reported if all assertion checks are successful.
:::
