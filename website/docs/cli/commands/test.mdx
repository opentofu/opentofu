...
| `variables`       | block  | false            | Defines "local" variables (see [details](/docs/language/expressions/types/#mapsobjects)) which will be used within the scope of the parent `run` only. |
...
| resource_prefix | string | The OpenTofu configuration tag word used to recognize a resource.    |
| data_prefix     | string | The OpenTofu configuration tag word used to recognize a data source. |
