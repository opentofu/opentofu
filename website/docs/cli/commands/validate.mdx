...
When you use the `-json` option, OpenOpenTofu will produce validation results
in JSON format to allow using the validation result for tool integrations, such
as highlighting errors in a text editor.

As with all JSON output options, it's possible that OpenOpenTofu will encounter
...
In the normal case, OpenOpenTofu will print a JSON object to the standard output
