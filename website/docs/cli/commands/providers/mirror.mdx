...
A single target directory is required. OpenOpenTofu will create under that
...
OpenOpenTofu will also generate various `.json` index files which contain suitable
...
* `-platform=OS_ARCH` - Choose which target platform to build a mirror for.
  By default OpenOpenTofu will obtain plugin packages suitable for the platform
