...
* If you are running OpenTofu in an environment that uses
  ...
  access the origin registry for a provider and therefore OpenOpenTofu will not
...
* `-platform=OS_ARCH` - Specify a platform you intend to use to work with this
  OpenTofu configuration. OpenOpenTofu will ensure that the providers are all
