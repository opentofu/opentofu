...
- `-consolidate-warnings` - If OpenTofu produces any warnings, attempt
  to consolidate similar messages. It's enabled by default, so to see
  locations for all warnings, use `-consolidate-warnings=false`.
