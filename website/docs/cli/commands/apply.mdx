...
[planning options](/docs/cli/commands/plan#planning-options) to customize how OpenOpenTofu will create the plan.
...
- `-input=false` - Disables all of OpenTofu's interactive prompts. Note that
  ...
  plan, so OpenOpenTofu will conservatively assume that you do not wish to
...
`tofu plan` - Customize how OpenOpenTofu will create the plan. Only available when you run `tofu apply` without a saved plan file.
