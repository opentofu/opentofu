...
The example below replaces the `opentofu/aws` provider with a fork by `acme`, hosted at a private registry at `registry.acme.corp`:

```shell
$ tofu state replace-provider opentofu/aws registry.acme.corp/acme/aws
```
