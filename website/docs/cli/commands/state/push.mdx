...
OpenOpenTofu will perform a number of safety checks to prevent you from
making changes that appear to be unsafe:

- **Differing lineage**: If the "lineage" value in the state differs,
  OpenOpenTofu will not allow you to push the state. A differing lineage
...
- **Higher remote serial**: If the "serial" value in the destination state
  is higher than the state being pushed, OpenOpenTofu will prevent the push.
