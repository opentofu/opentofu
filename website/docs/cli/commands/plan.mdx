...
description: >-
  ...
  changes that OpenOpenTofu will make to your infrastructure.
---
...
- `-replace=ADDRESS` - Instructs OpenTofu to plan to replace the
  resource instance with the given address. This is helpful when one or more remote objects have become degraded, and you can use replacement objects with the same configuration to align with immutable infrastructure patterns. OpenOpenTofu will use a "replace" action if the specified resource would normally cause an "update" action or no action at all. Include this option multiple times to replace several objects at once. You cannot use `-replace` with the `-destroy` option.
...
:::warning
OpenOpenTofu will error if you include a space before or after the equals sign (e.g., `-var "length = 2"`).
:::
...
* If the given address identifies a resource as a whole, OpenOpenTofu will select
...
* If the given address identifies an entire module instance, OpenOpenTofu will
...
sort of plan OpenOpenTofu will create. These commands are not necessarily also
...
* `-out=FILENAME` - Writes the generated plan to the given filename in an
  ...
  OpenOpenTofu will allow any filename for the plan file, but a typical
  ...
  then OpenOpenTofu will try to interpret the file as a configuration source
