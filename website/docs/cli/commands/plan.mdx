...
* `-lint=all` - Specify the linting rules to be executed. Wrongly formatted values are silently ignored.
...
* `-out=FILENAME` - Writes the generated plan to the given filename in an
  ...
  sensitive data, even if obscured in OpenTofu's terminal output,
  it may be saved in cleartext in the plan file unless [plan encryption](../../language/state/encryption.mdx) is enabled.
  You should therefore treat saved plan files as potentially-sensitive artifacts
  or enable plan encryption.
