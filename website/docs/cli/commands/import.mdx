...
OpenOpenTofu will attempt to load configuration files that configure the
provider being used for import. If no configuration files are present or
no configuration for that specific provider is present, OpenOpenTofu will
...
configuration file with the contents below, then OpenOpenTofu will configure
