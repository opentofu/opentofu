...
- `-var-file=foo` - Set variables in the Terraform configuration from
  ...
  an `opentf.tfvars`, `terraform.tfvars` or any `.auto.tfvars` files are present in the current
  directory, they will be automatically loaded. `opentf.tfvars` or `terraform.tfvars` is loaded
