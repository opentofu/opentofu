...
:::warning
The `destroy` flag is persisted in the state file, even when resources are removed from the configuration.
...
Alternatively, you can add the `removed` block for the removed resource with `lifecycle.destroy = true`, which will override the `destroy` flag in the state file.
:::
