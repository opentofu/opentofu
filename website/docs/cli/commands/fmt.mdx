...
For providing a better user experience, this command is having an external dependency on `diff`. Ensure that this is installed on the system where you use `tofu fmt` command.