...
As of OpenTofu 1.12, you can also specify instances of a resource or data source which use a `for_each` or `count`. You can even specify all of them, using a wildcard instance key. In the example below, we specify a series of tests; the comments above the tests explain whether or not they actually pass and why.
...
As mentioned before, you can specify an override in either the `mock_provider` block or at the root level of a test file. Root-level overrides always take precedence over anything specified in the `mock_provider`, as you can see in this example.
