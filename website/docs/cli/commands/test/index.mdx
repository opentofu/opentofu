...
import MockResourceMain from '!!raw-loader!./examples/mock_resource/main.tf'
import MockResourceTest from '!!raw-loader!./examples/mock_resource/main.tftest.hcl'
...
* The **[`mock_resource` and `mock_data` blocks](#the-mock_resource-and-mock_data-blocks)** (optional): define default values for computed attributes of mocked resources and data sources.
...
Cross-run references (e.g. `run.setup.filename`) only resolve after `run` blocks that use `command = apply` (the default). Run blocks with `command = plan` do not populate the run state needed to make `run.*` references available to later blocks. If you need to pass outputs between run blocks, ensure the producing run block uses `command = apply`. See [#3754](https://github.com/opentofu/opentofu/issues/3754) for more details.
...
[provider instances](../../../language/providers/configuration/#for_each-multiple-instances-of-a-provider-configuration) on the test suite.
...
### The `mock_resource` and `mock_data` blocks

These blocks are used inside a `mock_provider` block to customize mocked values for a specific resource or data source type.

When using `mock_provider`, OpenTofu skips real provider calls and generates placeholder values for computed attributes.
Use `mock_resource` and `mock_data` to set default values for computed attributes/blocks for a given resource or data source type, instead of relying on auto generated placeholders.

A `mock_resource` / `mock_data` block affects **all instances** of the given type that are handled by the surrounding `mock_provider`.

:::note

Defaults can only be set for **computed** attributes/blocks; configuration values cannot be changed via mocks.
...
| Name     | Type         | Description                                                                                                                                                         |
|:--------:|:------------:|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| defaults | map          | Optional. Default values for computed attributes and blocks for this type. If omitted, OpenTofu auto generates values.  |

:::note

`mock_resource` and `mock_data` blocks are overriding the entire resource or data source types. If you want to override a specific instance of a resource, use the `override_resource` or `override_data` blocks. When both are defined for the same resource instance, `override_resource`/`override_data` takes precedence because it is more specific.

:::

In the example below, we use mock_resource and mock_data to provide stable defaults for computed attributes, while verifying that the mock defaults are applied correctly — all without making real provider API calls:

<Tabs>
    <TabItem value={"test"} label={"main.tftest.hcl"} default>
        <CodeBlock language={"hcl"}>{MockResourceTest}</CodeBlock>
    </TabItem>
    <TabItem value={"main"} label={"main.tf"}>
        <CodeBlock language={"hcl"}>{MockResourceMain}</CodeBlock>
    </TabItem>
</Tabs>
