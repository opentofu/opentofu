...
Provider-defined functions are also automatically mocked. If they are being used and the provider
mocked is using `configuration_aliases` or `alias`, the function should be called using [provider aliases](../../../language/functions/#provider-defined-functions).
