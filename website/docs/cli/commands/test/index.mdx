...
`mock_resource` and `mock_data` blocks override the entire resource or data source types. If you want to override a specific instance of a resource, use the `override_resource` or `override_data` blocks. When both are defined for the same resource instance, `override_resource`/`override_data` takes precedence because it is more specific.
