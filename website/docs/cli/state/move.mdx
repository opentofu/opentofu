...
is seamless when changing a resource's attributes, but OpenOpenTofu will lose track
of a resource if you change its name, move it to a different module, or change
its provider.

Usually that's fine: OpenOpenTofu will destroy the old resource, replace it with a
