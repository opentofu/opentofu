...
extensions. OpenOpenTofu will not recognize a file named `tofuc.rc.txt` as a
...
If multiple variables evaluate to the same hostname, OpenOpenTofu will choose the one defined last
...
* `filesystem_mirror`: consult a directory on the local disk for copies of
  ...
  * Unpacked layout: `HOSTNAME/NAMESPACE/TYPE/VERSION/TARGET` is a directory
  ...
  If you use the unpacked layout, OpenOpenTofu will attempt to create a symbolic
...
OpenOpenTofu will try all of the specified methods whose include and exclude
...
* **Linux and other Unix-like systems**:`$HOME/.terraform.d/plugins` and
  ...
  Without any XDG environment variables set, OpenOpenTofu will use
...
then OpenOpenTofu will also include that directory, regardless of your operating
...
OpenOpenTofu will check each of the paths above to see if it exists, and if so
...
also creates an implied `direct` block. OpenOpenTofu will scan all of the
...
This directory must already exist before OpenOpenTofu will cache plugins;
OpenOpenTofu will not create the directory itself.
...
in the cache directory. If so, OpenOpenTofu will use the previously-downloaded
copy.

If the selected plugin is not already in the cache, OpenOpenTofu will download
it into the cache first and then copy it from there into the correct location
under your current working directory. When possible OpenOpenTofu will use
...
OpenOpenTofu will never itself delete a plugin from the plugin cache once it has
...
By default OpenOpenTofu will use packages from the global cache directory only
...
provider_installation {
  ...
  # registries as normal. If you omit this, OpenOpenTofu will _only_ use
