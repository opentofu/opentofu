...
like `registry.opentofu.org/opentofu/aws`. For convenience in the common case,
...
`registry.opentofu.org/opentofu/*` but also excluding
`registry.opentofu.org/opentofu/dns` will make that installation method apply
to everything in the `opentofu` namespace with the exception of
`opentofu/dns`.
...
`registry.opentofu.org/opentofu/*` and `opentofu/*` are equivalent.
...
provider_installation {
  ...
  # for the opentofu/null provider. This disables the version and checksum
  # verifications for this provider and forces OpenTofu to look for the
  # null provider plugin in the given directory.
  dev_overrides {
    "opentofu/null" = "/home/developer/tmp/opentofuf-null"
  }
