...
Our first prerelease for each release series marks that series being feature-complete, and so before we merge any more PRs we must create the new release branch, named after the first two segments of the release version number: `vX.Y.0`.
